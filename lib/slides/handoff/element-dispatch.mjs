...
async function handleRichText(slide, el, ctx) {
  applyRotation(slide.addText(richTextRuns(el.runs ?? []), textOpts(el, ctx)), el);
}
...
function svgCtmAt(spans, pos, ownRaw) {
  ...
  for (const s of spans) {
    if (s.transform && s.start <= pos && pos < s.end) mats.push(s.transform);
  }
...
export function parseSvgShapes(innerMarkup) {
  const shapes = [];
  const stripped = innerMarkup.replace(/<!--[\s\S]*?-->/g, '');
  const gradients = parseSvgGradients(stripped);
  const groupSpans = scanSvgGroupSpans(stripped);
  ...
  while ((m = tagRe.exec(stripped)) !== null) {
    ...
    if (ctm && shapes.length > shapeStart) shapes[shapes.length - 1].ctm = ctm;
  }
