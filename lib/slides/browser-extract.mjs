export async function extractSlides(page, opts = {}) {
  // No `process` here: this module is loaded by the environment-agnostic core,
  // where reading it would throw at call time rather than import time. The host
  // reads the OPENFIG_FLEX_AUTO_LAYOUT env var and passes the result in.
  const flexAutoLayout = Boolean(opts.flexAutoLayout);
  // `document` and `getComputedStyle` come in through `realm`, never as free
  // variables. Playwright bound them correctly by accident — it serialises this
  // function and re-parses it inside the page — but a host that calls the
  // function object directly (the iframe surface) would bind them to *its* DOM
  // and measure the wrong document without throwing. See host-contract.mjs.
  return await page.evaluate(({ realm, arg }) => {
    const { document, getComputedStyle } = realm;
    const flexAutoLayoutEnabled = !!arg.flexAutoLayout;
    const CANVAS_W = 1920;
    const CANVAS_H = 1080;

    function decodeSvgDataUrl(src) {
      if (typeof src !== 'string') return null;
      const m = src.match(/^data:image\/svg\+xml(?:;charset=[^;,]+)?(;base64)?,(.*)$/i);
      if (!m) return null;
      try {
        return m[1] ? atob(m[2]) : decodeURIComponent(m[2]);
      } catch {
        return null;
      }
    }

    const sections = Array.from(document.querySelectorAll('section'));
    if (sections.length === 0) return { slides: [] };

    const results = [];
    for (let i = 0; i < sections.length; i++) {
      const sec = sections[i];
      const saved = sections.map((s) => ({
        el: s,
        display: s.style.display,
        visibility: s.style.visibility,
        position: s.style.position,
        top: s.style.top,
        left: s.style.left,
        width: s.style.width,
        height: s.style.height,
        overflow: s.style.overflow,
      }));
      for (const s of sections) {
        if (s === sec) {
          s.style.display = 'block';
          s.style.visibility = 'visible';
          s.style.position = 'absolute';
          s.style.top = '0px';
          s.style.left = '0px';
          s.style.width = CANVAS_W + 'px';
          s.style.height = CANVAS_H + 'px';
          s.style.overflow = 'visible';
        } else {
          s.style.display = 'none';
        }
      }
      document.body.offsetHeight;
      results.push(collectSection(sec, i));
      for (const { el, display, visibility, position, top, left, width, height, overflow } of saved) {
        el.style.display = display;
        el.style.visibility = visibility;
        el.style.position = position;
        el.style.top = top;
        el.style.left = left;
        el.style.width = width;
        el.style.height = height;
        el.style.overflow = overflow;
      }
    }

    // Collect CSS custom properties from :root so the Node side can
    // resolve `var(--name)` references that Chromium leaves literal in
    // inline SVG attributes (e.g. fill="var(--accent)"). getComputedStyle
    // on the documentElement exposes every declared `--foo` in resolved
    // form.
    const cssVars = {};
    const rootCs = getComputedStyle(document.documentElement);
    for (let i = 0; i < rootCs.length; i++) {
      const prop = rootCs[i];
      if (prop.startsWith('--')) {
        cssVars[prop] = rootCs.getPropertyValue(prop).trim();
      }
    }

    return { slides: results, cssVars };

    function collectSection(root, index) {
      const secRect = root.getBoundingClientRect();
      const off = { x: secRect.left, y: secRect.top };
      const elements = [];
      const warnings = [];

      // currentTarget is the array that emissions push into. Defaults to the
      // slide's top-level `elements`. When walk() enters a flex/grid element
      // (and flexAutoLayoutEnabled is on), it creates a `layoutContainer` node,
      // pushes it to currentTarget, then routes descendant emissions into the
      // container's own `children` array. Elements with position:absolute/fixed
      // escape back to slide top-level regardless of surrounding containers.
      let currentTarget = elements;
      // Absolute descendants normally escape to the slide root. A CSS mask is
      // a compositing boundary, though: every descendant must stay inside the
      // masked group even when it is absolutely positioned.
      let absoluteTarget = elements;
      function pushElement(e) { currentTarget.push(e); }

      walk(root);
      insetTextsBehindLeftMarkers(elements);

      return {
        index,
        dataLabel: root.getAttribute('data-label'),
        background: getComputedStyle(root).backgroundColor,
        elements,
        warnings,
      };

      // Walk the emitted tree (top-level + nested layoutContainer children)
      // looking for bullet-marker ellipses whose left edge collides with a
      // text element's left edge. When found, inset the text past the marker
      // so the dot is not visually overlapped by the first word. We only move
      // the left edge — preserving the text's right edge and width preserves
      // the source's wrapping intent (e.g. an orphan <p> intended to span
      // full slide width remains full-width).
      function insetTextsBehindLeftMarkers(nodes) {
        const BULLET_GAP = 10;
        const inflow = (nodes ?? []).filter(Boolean);
        for (const marker of inflow) {
          if (!marker || !marker._leftMarker) continue;
          const my0 = marker.y;
          const my1 = marker.y + marker.height;
          const mx0 = marker.x;
          const mx1 = marker.x + marker.width;
          for (const el of inflow) {
            if (el === marker) continue;
            if (el.type !== 'text' && el.type !== 'richText') continue;
            if (typeof el.x !== 'number' || typeof el.width !== 'number') continue;
            const ey0 = el.y;
            const ey1 = el.y + (el.height ?? 0);
            const verticalOverlap = Math.min(ey1, my1) - Math.max(ey0, my0);
            if (verticalOverlap <= 0) continue;
            if (Math.abs(el.x - mx0) > 2) continue;
            const inset = mx1 - el.x + BULLET_GAP;
            el.x = el.x + inset;
            el.width = Math.max(0, el.width - inset);
          }
        }
        for (const el of inflow) {
          if (el && Array.isArray(el.children)) {
            insetTextsBehindLeftMarkers(el.children);
          }
          if (el) delete el._leftMarker;
        }
      }

      function toLocal(rect) {
        return {
          x: rect.left - off.x,
          y: rect.top - off.y,
          width: rect.width,
          height: rect.height,
        };
      }

      function walk(el) {
        if (!el || el.nodeType !== 1) return;
        const tag = el.tagName.toUpperCase();
        if (tag === 'SCRIPT' || tag === 'STYLE' || tag === 'TEMPLATE' || tag === 'HEAD') return;

        const cs = getComputedStyle(el);
        if (cs.display === 'none' || cs.visibility === 'hidden') return;

        const rect = toLocal(el.getBoundingClientRect());
        if (rect.width === 0 && rect.height === 0 && tag !== 'SECTION') {
          return;
        }

        // Absolute-positioned subtrees escape any surrounding Auto Layout
        // container: their emissions (and descendants') route to slide root
        // so their authored x/y coordinates are preserved verbatim.
        const positionKind = cs.position;
        const escapesContainer = positionKind === 'absolute' || positionKind === 'fixed';
        const savedTargetEntry = currentTarget;
        const savedAbsoluteTarget = absoluteTarget;
        if (escapesContainer) currentTarget = absoluteTarget;

        // A CSS mask applies to the element and its entire composited subtree.
        // Preserve that boundary in the handoff instead of flattening the
        // descendants and losing the mask. Only a single linear alpha mask is
        // currently expressible as one editable native Figma mask node.
        const maskLayer = tag === 'SECTION' ? null : parseLinearMaskImage(cs, el);
        if (maskLayer) {
          const masked = {
            type: 'maskedGroup',
            x: rect.x, y: rect.y, width: rect.width, height: rect.height,
            mask: maskLayer,
            children: [],
          };
          pushElement(masked);
          currentTarget = masked.children;
          absoluteTarget = masked.children;
        }
        try {

        emitPseudo(el, cs, '::before', rect);

        if (tag === 'IMG') {
          const src = el.getAttribute('src');
          if (src) {
            // <img src="data:image/svg+xml;..."> holds vector wordmark/logo
            // paths inline. Routing it through the raster image pipeline
            // bakes it to pixels at the current display size, and any zoom
            // in Figma shows aliasing. Decode the data URL and re-emit as
            // a native SVG so the handoff turns it into Figma VECTOR nodes.
            const svgInline = decodeSvgDataUrl(src);
            if (svgInline) {
              const vb = svgInline.match(/viewBox\s*=\s*(['"])([^'"]+)\1/i);
              warnUnsupportedSvg(svgInline, el);
              pushElement({
                type: 'svg',
                x: rect.x, y: rect.y, width: rect.width, height: rect.height,
                viewBox: vb ? vb[2] : `0 0 ${rect.width} ${rect.height}`,
                inline: svgInline,
                ...(cs.color ? { color: cs.color } : {}),
                ...(parseFloat(cs.opacity) < 1 ? { opacity: parseFloat(cs.opacity) } : {}),
                ...(cs.filter && cs.filter !== 'none' ? { filter: cs.filter } : {}),
              });
            } else {
              pushElement({
                type: 'image',
                ...(figmaBlendMode(cs.mixBlendMode) ? { blendMode: figmaBlendMode(cs.mixBlendMode) } : {}),
                src,
                x: rect.x, y: rect.y, width: rect.width, height: rect.height,
                objectFit: cs.objectFit || 'contain',
                objectPosition: cs.objectPosition || '50% 50%',
                opacity: cs.opacity,
                ...(cs.filter && cs.filter !== 'none' ? { filter: cs.filter } : {}),
              });
            }
          } else {
            warnings.push({ msg: '<img> without src — dropped', sample: elPath(el) });
          }
        } else if (tag === 'SVG' || tag === 'svg') {
          // CSS `opacity` cascades as a group effect — the SVG's own computed
          // opacity is 1, but an ancestor like `.s1-deco { opacity: 0.12 }`
          // fades everything inside. Climb to the slide <section> multiplying
          // opacities so decorative SVGs render at the right visual weight.
          let eff = 1;
          for (let a = el; a && a.tagName?.toUpperCase() !== 'SECTION'; a = a.parentElement) {
            const ocs = getComputedStyle(a);
            const op = parseFloat(ocs.opacity);
            if (!Number.isNaN(op)) eff *= op;
          }
          warnUnsupportedSvg(el.outerHTML, el);
          pushElement({
            type: 'svg',
            x: rect.x, y: rect.y, width: rect.width, height: rect.height,
            viewBox: el.getAttribute('viewBox') || `0 0 ${rect.width} ${rect.height}`,
            inline: el.outerHTML,
            ...(cs.color ? { color: cs.color } : {}),
            ...(eff < 1 ? { opacity: eff } : {}),
            ...(cs.filter && cs.filter !== 'none' ? { filter: cs.filter } : {}),
          });
        } else {
          if (tag !== 'SECTION') maybeEmitShape(el, cs, rect);
          collectWarnings(el, cs);

          if (tag === 'SECTION') {
            for (const c of el.children) walk(c);
          } else {
            const role = classifyTextRole(el);
            if (role === 'leaf') {
              emitTextLeaf(el, cs, rect);
            } else if (role === 'mixed-inline') {
              emitInlineRuns(el, cs);
            } else {
              // role === 'container'. When enabled and the element lays out
              // its children via flex or single-axis grid, emit a
              // `layoutContainer` node and route descendant emissions into its
              // own `children` array. The handoff renders this as a Figma
              // Auto Layout frame; Chromium↔Figma glyph-metric divergence is
              // absorbed by re-flow inside the frame instead of causing a
              // child to cross a sibling's pre-computed coordinate.
              const wrap = flexAutoLayoutEnabled
                ? maybeBuildLayoutContainer(el, cs, rect)
                : null;
              if (wrap) {
                pushElement(wrap);
                const savedChildTarget = currentTarget;
                currentTarget = wrap.children;
                try {
                  for (const c of el.children) walk(c);
                  emitDirectTextInContainer(el, cs);
                } finally {
                  currentTarget = savedChildTarget;
                }
                // Reconcile the declared CSS `gap` against the actual
                // inter-emitted-child spacing. DOM children can be back-to-
                // back with gap=0 while the text RUNS inside them are
                // visually separated by padding / fixed-width / margins.
                // Auto Layout would otherwise pack the emitted text runs
                // with no space (e.g. "1Executive Summary"). Use the
                // measured spacing between emitted siblings instead.
                reconcileActualGap(wrap);
                // Post-emit overlap check. The pre-emit childrenMatchDeclaredGap
                // only sees the DOM children of `el`. But when a DOM child is
                // itself a composite (e.g. an icon whose background ellipse +
                // rotated bars both end up in our layoutContainer.children via
                // the maybeEmitShape + child-walk path), the EMITTED children
                // can overlap even if the DOM children don't. If so, unwrap.
                if (emittedChildrenOverlap(wrap)) {
                  const idx = savedChildTarget.lastIndexOf(wrap);
                  if (idx !== -1) {
                    savedChildTarget.splice(idx, 1, ...wrap.children);
                  }
                }
              } else {
                for (const c of el.children) walk(c);
                // Flex/block containers can also hold direct text adjacent to
                // block children (e.g. a legend row with a colored swatch div
                // plus a plain "Measured demand" label). The for-children loop
                // above only visits Element nodes, so direct text would otherwise
                // be lost. Emit each non-empty text node as its own text leaf.
                emitDirectTextInContainer(el, cs);
              }
            }
          }
        }

        emitPseudo(el, cs, '::after', rect);

        } finally {
          currentTarget = savedTargetEntry;
          absoluteTarget = savedAbsoluteTarget;
        }
      }

      // Returns a layoutContainer descriptor for a flex / single-axis-grid
      // element, or null if the element should be emitted flat (2-D grid,
      // unmapped justify-content). The returned object has an empty
      // `children` array that the caller fills by recursion.
      function maybeBuildLayoutContainer(el, cs, rect) {
        const d = cs.display;
        const isFlex = d === 'flex';
        // Grids are intentionally NOT wrapped. CSS grid with
        // `grid-template-columns: auto 1fr` and implicit rows is effectively
        // 2-D — Figma Auto Layout has no 2-D equivalent. Wrapping flattens
        // children into a single axis, which wrecks charts whose axis labels
        // sit at authored x/y inside the grid cell. Flex is a direct
        // analogue of Auto Layout; grid is not.
        if (!isFlex) return null;

        const direction = (cs.flexDirection || '').startsWith('column') ? 'COLUMN' : 'ROW';

        const gap = parseFloat(cs.rowGap || cs.columnGap || cs.gap || '0') || 0;
        const primary = mapJustifyContent(cs.justifyContent);
        if (primary === null) return null; // unsupported; fall back to flat

        // Only wrap when the authored flex layout is actually uniform. A flex
        // container used as a positioning scaffold (e.g. a chart whose title,
        // SVG, and legend sit at irregular y-offsets inside it) will have
        // children whose inter-gaps diverge from the declared `gap` property.
        // Auto Layout imposes uniform spacing, so wrapping these breaks
        // visual composition. Fall back to flat emission in that case.
        if (!childrenMatchDeclaredGap(el, direction, gap)) return null;

        // Skip wrapping when the flex subtree contains an SVG. handleSvg
        // decomposes the SVG into per-shape addText / addPath calls, so
        // inside an Auto Layout frame each shape becomes its own Auto Layout
        // sibling — chart axis labels end up stacked vertically instead of
        // positioned inside the chart. These containers are almost always
        // charts or diagrams that rely on internal absolute positioning.
        if (containsSvgDescendant(el)) return null;

        return {
          type: 'layoutContainer',
          direction,
          gap,
          x: rect.x, y: rect.y, width: rect.width, height: rect.height,
          paddingTop: pxFromCs(cs.paddingTop),
          paddingRight: pxFromCs(cs.paddingRight),
          paddingBottom: pxFromCs(cs.paddingBottom),
          paddingLeft: pxFromCs(cs.paddingLeft),
          primaryAxisAlignItems: primary,
          counterAxisAlignItems: mapAlignItems(cs.alignItems),
          children: [],
        };
      }

      // CSS align-items → Figma counterAxisAlignItems.
      // Figma has MIN / CENTER / MAX / BASELINE. CSS `baseline` maps directly.
      // CSS `stretch` has no Figma equivalent on counter-axis sizing mode; we
      // fall back to MIN (top) which is the closest neutral.
      function mapAlignItems(ai) {
        switch (ai) {
          case 'center': return 'CENTER';
          case 'flex-end':
          case 'end':
            return 'MAX';
          case 'baseline':
          case 'first baseline':
          case 'last baseline':
            return 'BASELINE';
          case 'flex-start':
          case 'start':
          case 'stretch':
          default:
            return 'MIN';
        }
      }

      // Returns true iff the in-flow children of `el` are spaced along
      // `direction` with a gap within 8px of the declared `gap`. Children
      // with position:absolute/fixed are excluded — they escape to slide root
      // in walk(). A single in-flow child trivially matches. Elements with
      // no in-flow children are considered non-auto-layoutable (the wrapping
      // adds no value and risks breaking other layers).
      function childrenMatchDeclaredGap(el, direction, gap) {
        const inflow = [];
        for (const c of el.children) {
          const ccs = getComputedStyle(c);
          if (ccs.display === 'none' || ccs.visibility === 'hidden') continue;
          if (ccs.position === 'absolute' || ccs.position === 'fixed') continue;
          inflow.push(c);
        }
        // A flex container with fewer than 2 in-flow children serves no
        // Auto Layout purpose — there is nothing to flow against. Wrapping
        // the lone child adds a phantom parent frame whose hover-bounds are
        // bigger than the child's select-bounds, which reads as a bug in
        // Figma. Fall back to flat emission so the child is placed at its
        // own rect.
        if (inflow.length < 2) return false;
        const axisStart = direction === 'ROW' ? 'left' : 'top';
        const axisSize = direction === 'ROW' ? 'width' : 'height';
        for (let i = 1; i < inflow.length; i++) {
          const prev = inflow[i - 1].getBoundingClientRect();
          const cur = inflow[i].getBoundingClientRect();
          const prevEnd = prev[axisStart] + prev[axisSize];
          const curStart = cur[axisStart];
          const actual = curStart - prevEnd;
          // Overlap on the flex axis means the container is being used as a
          // positioning scaffold (e.g. an icon where a red dot has two
          // rotated bars overlaid to form an X). Auto Layout would lay those
          // overlapping children out as side-by-side siblings, splitting the
          // icon into a scattered row. Fall back to flat emission.
          if (actual < -2) return false;
          if (Math.abs(actual - gap) > 8) return false;
        }
        return true;
      }

      // After children are emitted, replace the CSS-declared `gap` with
      // the actual measured inter-child spacing along the flex axis. This
      // handles the common pattern where a flex container has gap=0 but
      // children have internal padding / min-width / fixed-width that
      // creates visual spacing between the TEXT RUNS inside them. Without
      // this step, Auto Layout packs text runs back-to-back and collapses
      // authored visual gaps (e.g. TOC "1   Executive Summary" rendering
      // as "1Executive Summary"). Uses the median of pairwise gaps so an
      // odd outlier doesn't skew the result.
      function reconcileActualGap(wrap) {
        const kids = wrap.children ?? [];
        if (kids.length < 2) return;
        const axisStart = wrap.direction === 'ROW' ? 'x' : 'y';
        const axisSize = wrap.direction === 'ROW' ? 'width' : 'height';
        const gaps = [];
        for (let i = 1; i < kids.length; i++) {
          const prev = kids[i - 1];
          const cur = kids[i];
          if (typeof prev?.[axisStart] !== 'number') return;
          if (typeof cur?.[axisStart] !== 'number') return;
          const g = cur[axisStart] - (prev[axisStart] + (prev[axisSize] ?? 0));
          if (g < 0) return; // overlap — let emittedChildrenOverlap unwrap
          gaps.push(g);
        }
        if (gaps.length === 0) return;
        gaps.sort((a, b) => a - b);
        wrap.gap = Math.round(gaps[Math.floor(gaps.length / 2)]);
      }

      // After emission, verify no two children overlap on the flex axis.
      // Auto Layout would lay overlapping children out as side-by-side
      // siblings, which breaks composite icons (e.g. a red ✗ made from a
      // circle + two rotated bars: the bars overlap the circle on the ROW
      // axis). Nested layoutContainers are checked by their own x/width
      // (not their descendants').
      function emittedChildrenOverlap(wrap) {
        const children = wrap.children ?? [];
        if (children.length < 2) return false;
        const axisStart = wrap.direction === 'ROW' ? 'x' : 'y';
        const axisSize = wrap.direction === 'ROW' ? 'width' : 'height';
        for (let i = 1; i < children.length; i++) {
          const prev = children[i - 1];
          const cur = children[i];
          if (typeof prev?.[axisStart] !== 'number') continue;
          if (typeof cur?.[axisStart] !== 'number') continue;
          const prevEnd = prev[axisStart] + (prev[axisSize] ?? 0);
          const curStart = cur[axisStart];
          if (curStart - prevEnd < -2) return true;
        }
        return false;
      }

      function containsSvgDescendant(el) {
        if (!el) return false;
        if (el.querySelector && el.querySelector('svg')) return true;
        return false;
      }

      function mapJustifyContent(jc) {
        switch ((jc || 'flex-start').trim()) {
          case 'flex-start':
          case 'start':
          case 'left':
          case 'normal':
            return 'MIN';
          case 'center':
            return 'CENTER';
          case 'flex-end':
          case 'end':
          case 'right':
            return 'MAX';
          case 'space-between':
            return 'SPACE_BETWEEN';
          default:
            return null; // space-around, space-evenly → fall back
        }
      }

      function classifyTextRole(el) {
        let hasDirectText = false;
        let hasBlockChild = false;
        let hasInlineChild = false;
        let hasVisualChild = false;

        for (const c of el.childNodes) {
          if (c.nodeType === 3) {
            if (c.textContent && c.textContent.trim()) hasDirectText = true;
          } else if (c.nodeType === 1) {
            const ct = c.tagName.toUpperCase();
            if (ct === 'BR') continue;
            if (ct === 'SCRIPT' || ct === 'STYLE' || ct === 'TEMPLATE') continue;
            const ccs = getComputedStyle(c);
            if (ccs.display === 'none') continue;
            if (ct === 'SVG' || ct === 'IMG' || ct === 'CANVAS' || ct === 'VIDEO') {
              hasVisualChild = true;
            }
            // Empty inline element whose geometry + fill/border come purely
            // from CSS (e.g. a colour swatch <span>) is a visual child too.
            // Without this, an ancestor like <td> classifies as a text "leaf"
            // and emitTextLeaf silently drops the span without walking it,
            // so the swatch never emits a rect.
            if (isCssOnlyVisual(c, ccs)) hasVisualChild = true;
            if (['inline', 'inline-block', 'inline-flex'].includes(ccs.display)) {
              hasInlineChild = true;
            } else {
              hasBlockChild = true;
            }
          }
        }

        if (hasBlockChild) return 'container';
        if (hasVisualChild) return 'container';
        if (!hasDirectText && !hasInlineChild) return 'container';

        if (hasInlineChild && divergent(el)) return 'mixed-inline';
        return 'leaf';
      }

      function isCssOnlyVisual(c, ccs) {
        const text = (c.textContent || '').trim();
        if (text) return false;
        // Has the element got any non-text children that would themselves
        // render? If so leave classification to the normal path.
        for (const cc of c.childNodes) {
          if (cc.nodeType === 1) return false;
        }
        const w = pxFromCs(ccs.width);
        const h = pxFromCs(ccs.height);
        if (w <= 0 || h <= 0) {
          const rect = c.getBoundingClientRect && c.getBoundingClientRect();
          if (!rect || rect.width <= 0 || rect.height <= 0) return false;
        }
        if (isVisibleColor(ccs.backgroundColor)) return true;
        const borderW = pxFromCs(ccs.borderTopWidth) + pxFromCs(ccs.borderRightWidth)
          + pxFromCs(ccs.borderBottomWidth) + pxFromCs(ccs.borderLeftWidth);
        if (borderW > 0) return true;
        return false;
      }

      function divergent(el) {
        const pcs = getComputedStyle(el);
        const ref = {
          fontSize: pcs.fontSize,
          // Family belongs here with the rest. A child that differs from its
          // parent *only* in family used to leave the block classified as a
          // leaf, which measures the whole thing in the parent's computed
          // style and discards the child's family entirely — the run never
          // reaches the per-run path that would have preserved it.
          fontFamily: pcs.fontFamily,
          color: pcs.color,
          fontWeight: pcs.fontWeight,
          fontStyle: pcs.fontStyle,
        };
        for (const c of el.childNodes) {
          if (c.nodeType !== 1) continue;
          if (c.tagName.toUpperCase() === 'BR') continue;
          const ccs = getComputedStyle(c);
          if (ccs.display === 'none') continue;
          if (
            ccs.fontSize !== ref.fontSize ||
            ccs.fontFamily !== ref.fontFamily ||
            ccs.color !== ref.color ||
            ccs.fontWeight !== ref.fontWeight ||
            ccs.fontStyle !== ref.fontStyle
          ) {
            return true;
          }
        }
        return false;
      }

      function countTextLines(el) {
        const range = document.createRange();
        range.selectNodeContents(el);
        const rects = range.getClientRects();
        if (rects.length <= 1) return rects.length;
        // `countLines`, not a Set of tops — see the note on that function.
        return countLines(rects);
      }

      // For elements classified as 'container' that also carry direct text
      // nodes in childNodes (flex row with an element sibling + a raw text
      // label). Each non-empty direct text node becomes its own text leaf,
      // positioned via a Range on that node so the rect is tight to the
      // rendered glyphs. Whitespace-only text nodes are skipped.
      function emitDirectTextInContainer(el, cs) {
        for (const c of el.childNodes) {
          if (c.nodeType !== 3) continue;
          const text = collapseText(c.textContent);
          if (!text) continue;
          const range = document.createRange();
          range.selectNode(c);
          const tr = range.getBoundingClientRect();
          if (tr.width === 0 && tr.height === 0) continue;
          const style = textStyle(cs);
          const el2 = {
            type: 'text',
            text,
            x: tr.left - off.x,
            y: tr.top - off.y,
            width: tr.width,
            height: tr.height,
            ...style,
          };
          // Text next to a block sibling in a flex row is effectively
          // single-line; prevent Figma from wrapping it.
          if (countLines(range.getClientRects()) <= 1
            && !alignmentIsLoadBearing(style.align, tr.width, tr.width)) el2.noWrap = true;
          const rot = verticalTextRotation(cs);
          if (rot) applyTextRotation(el2, rot);
          pushElement(el2);
        }
      }

      function emitTextLeaf(el, cs, rect) {
        const hasHardBreak = [...el.childNodes].some(
          (c) => c.nodeType === 1 && c.tagName?.toUpperCase?.() === 'BR',
        );
        const text = hasHardBreak
          ? collapseTextPreservingBreaks(el.innerText ?? el.textContent ?? '')
          : collapseText(el.innerText ?? el.textContent ?? '');
        if (!text) return;
        const lines = countTextLines(el);
        const style = textStyle(cs);
        const range = document.createRange();
        range.selectNodeContents(el);
        const rangeRects = [...range.getClientRects()];
        const tr = range.getBoundingClientRect();
        const textW = tr.width;
        // Positioned by where the glyphs are, not where the box is.
        //
        // CSS puts the two in different places, in both directions. When
        // line-height is smaller than the font's own ascent plus descent, the
        // glyphs overflow *above* the box: an 80px number with tight leading
        // measured 10px of ink above its own box top. When line-height is
        // generous, half-leading pushes the ink down inside the box — 2px for
        // 22px text on a 30px line.
        //
        // Figma has no equivalent of either. It lays ink out from the box using
        // its own ascent, so whichever way CSS moved the glyphs relative to the
        // box, emitting the box top reproduces the offset as an error. Measured
        // against the design on one slide: the number sat 10px low, closing the
        // gap to its label from 25px to 13px, while the label and caption below
        // it rode 2 and 3px high. All three are the same rule with different
        // leading.
        //
        // The flex-row branch above already positions from the range rect. This
        // is that, in the branch most text actually takes.
        const inkY = tr.height > 0 ? tr.top - off.y : rect.y;
        const el2 = {
          type: 'text',
          text,
          x: rect.x, y: inkY, width: rect.width, height: rect.height,
          ...style,
        };
        if (lines === 1 && !alignmentIsLoadBearing(style.align, rect.width, textW)) {
          if (textW > 0 && Math.abs(textW - rect.width) < 2) {
            el2.noWrap = true;
          } else if (textW > 0 && textW < rect.width) {
            el2.x = tr.left - off.x;
            el2.y = tr.top - off.y;
            el2.width = tr.width;
            el2.height = tr.height;
            el2.noWrap = true;
          }
        } else if (hasHardBreak
          && !alignmentIsLoadBearing(
            style.align,
            rect.width,
            rangeRects.reduce((w, r) => Math.max(w, r.width), 0),
          )) {
          // If a segment between authored breaks ALSO soft-wraps inside its
          // box (more visual lines than \n-delimited segments), the box width
          // is load-bearing — e.g. "PARAMETER CLARITY" wrapping under
          // "FINDING 03" in a 200px label column. Leaving noWrap unset lets
          // Figma re-wrap within the measured width and reproduce that layout.
          // Forcing noWrap here would overflow the column into the next one.
          const visualLines = countLines(rangeRects);
          const authoredLines = text.split('\n').length;
          if (visualLines <= authoredLines) {
            let widest = 0;
            for (const r of rangeRects) if (r.width > widest) widest = r.width;
            if (widest > 0) {
              const padded = Math.ceil(widest * 1.08) + 4;
              if (padded > el2.width) el2.width = padded;
            }
            // Preserve authored line breaks while preventing Figma from
            // wrapping each line again with slightly different font metrics.
            el2.noWrap = true;
          }
        }
        // Give a wrapping paragraph a hair of margin so a line that only just
        // fitted in Chromium still fits in Figma. See wrapMargin.
        if (!el2.noWrap && lines > 1 && el2.width > 0) {
          el2.width += wrapMargin(pxFromCs(cs.fontSize));
        }
        const rot = verticalTextRotation(cs);
        if (rot) applyTextRotation(el2, rot);
        pushElement(el2);
      }

      function squishInline(s) {
        return String(s || '').replace(/\s+/g, ' ');
      }

      function emitInlineRuns(el, parentCs) {
        emitInlineAsRichText(el, parentCs);
      }

      // A margin wide enough to absorb the difference between Chromium's line
      // breaking and Figma's, and narrow enough that it cannot change where a
      // line breaks by itself.
      //
      // Measured on a real export. Chromium fitted "...(for example team" into
      // an 831px column with **0.17px to spare** — 0.021% headroom. Figma needed
      // only 0.02% more advance width for the same string, so it moved one word
      // to the next line and every line below it shifted. Nothing about that is
      // a wide-metrics problem; it is a knife edge, and any sub-pixel difference
      // in shaping or rounding flips it.
      //
      // The bound is what makes this safe rather than a guess. To pull an extra
      // word *up* a line, a margin would have to be as wide as a space plus the
      // shortest word — about 12px at 22px Inter, or roughly 0.5em. Staying
      // under a quarter of the font size leaves 2x headroom against that, so the
      // margin can only ever rescue lines that were already within a hair of
      // fitting. Capped at 8px so display sizes do not inherit a large one, and
      // floored at 1px so it always does something.
      //
      // This replaces "deliberately no padding", whose stated reason was that
      // widening a box bleeds into the next column. Measured on the slide that
      // prompted it, the gutter between those two columns is 60px — the margin
      // is 6px there, a tenth of the gap. The concern was real for the 1.08x
      // padding used elsewhere; it does not apply at this size.
      function wrapMargin(fontSizePx) {
        const size = fontSizePx > 0 ? fontSizePx : 16;
        return Math.min(8, Math.max(1, size * 0.25));
      }

      // Whether an explicit `text-align` is actually positioning the text, or is
      // just along for the ride.
      //
      // Aligned single-line text is normally left at its measured box width: a
      // right-aligned numeral in a table cell sits in a box far wider than
      // itself, and that box is what places it. Tightening those is the bug
      // commit 1abf846 had to work around.
      //
      // But a box that is already the width of its own text has no room for
      // alignment to mean anything — and, more to the point, no room to absorb
      // the difference between Chromium's metrics and Figma's. Measured on a
      // real export: a centred 22px Bold label 313.2px wide in a 313.2px box,
      // zero slack, which Figma wrapped onto two lines and dropped on top of the
      // caption below it.
      //
      // The tolerance is a judgement, not a measurement: 2px or 2%, chosen to
      // sit well below a table cell's slack and above the sub-percent drift
      // between the two engines' metrics. If a case turns up between those, the
      // honest fix is to measure the drift for the font in use rather than to
      // widen this number.
      function alignmentIsLoadBearing(align, boxWidth, textWidth) {
        if (!align) return false;
        if (!(textWidth > 0)) return true;
        return boxWidth - textWidth > Math.max(2, textWidth * 0.02);
      }

      // How many visual lines a set of client rects covers. The only line count
      // in this module: `countTextLines`, the flex-row branch and the hard-break
      // branch all route through here.
      //
      // Declared as a function rather than assigned to a const on purpose — it
      // is called from code that appears above it, and hoisting is what makes
      // that legal. A `const` here throws at run time inside the page.
      //
      // Counting distinct `top` values looks equivalent and is not: a <sub>,
      // <sup>, or any inline set at a different size sits at a different top
      // while sharing the line with its siblings. `847` followed by a
      // subscripted ` TWh` counted as two lines, so the run missed the
      // single-line branch that sets `noWrap`, and Figma — free to re-wrap —
      // put the unit on its own line on top of the caption below.
      //
      // Rects on one line overlap vertically; rects on different lines do not.
      function countLines(rects) {
        // Overlap has to be *substantial*, not merely non-zero. Consecutive
        // line boxes touch, and at a tight line-height they overlap by a pixel
        // or two — enough for an any-overlap test to merge them, and then the
        // merged band reaches the line after that, chaining every line into
        // one. A three-line label counted as one, lost its wrap, and pushed
        // out of its column: the same damage as the bug this function fixes,
        // in the opposite direction.
        //
        // A baseline-shifted inline shares most of its height with the line it
        // sits on; an adjacent line shares almost none. Half the shorter box is
        // comfortably between the two.
        const lines = [];
        const sorted = [...rects].filter((r) => r.width || r.height).sort((a, b) => a.top - b.top);
        for (const r of sorted) {
          const height = r.bottom - r.top;
          const hit = lines.find((l) => {
            const overlap = Math.min(r.bottom, l.bottom) - Math.max(r.top, l.top);
            if (overlap <= 0) return false;
            return overlap >= 0.5 * Math.min(height, l.bottom - l.top);
          });
          if (hit) {
            hit.top = Math.min(hit.top, r.top);
            hit.bottom = Math.max(hit.bottom, r.bottom);
          } else {
            lines.push({ top: r.top, bottom: r.bottom });
          }
        }
        return lines.length;
      }

      function emitInlineAsRichText(el, parentCs) {
        const runs = [];
        let hasHardBreak = false;
        // Track trailing margin-right of the previous element child so the
        // next sibling's run can be prefixed with a separator space. Without
        // this, e.g. `<span mr:8>•</span>Body` collapses to "•Body" with no
        // visible gap, even though Chrome renders 8px of space.
        let pendingGap = 0;
        for (const c of el.childNodes) {
          if (c.nodeType === 3) {
            const raw = squishInline(c.textContent);
            if (!raw) continue;
            const text = pendingGap > 0 && raw[0] !== ' ' ? ' ' + raw : raw;
            runs.push({
              text,
              ...runStyle(parentCs),
            });
            pendingGap = 0;
          } else if (c.nodeType === 1) {
            const ct = c.tagName.toUpperCase();
            if (ct === 'BR') { runs.push({ text: '\n' }); hasHardBreak = true; pendingGap = 0; continue; }
            if (ct === 'SCRIPT' || ct === 'STYLE' || ct === 'TEMPLATE') continue;
            const ccs = getComputedStyle(c);
            if (ccs.display === 'none' || ccs.visibility === 'hidden') continue;
            const raw = squishInline(c.textContent ?? '');
            if (!raw) continue;
            const myLeftGap = parseFloat(ccs.marginLeft) || 0;
            const totalGap = pendingGap + myLeftGap;
            const text = totalGap > 0 && raw[0] !== ' ' ? ' ' + raw : raw;
            runs.push({
              text,
              ...runStyle(ccs),
            });
            pendingGap = parseFloat(ccs.marginRight) || 0;
          }
        }
        if (runs.length === 0) return;
        const rect = toLocal(el.getBoundingClientRect());
        const el2 = {
          type: 'richText',
          runs,
          x: rect.x, y: rect.y, width: rect.width, height: rect.height,
          ...textStyle(parentCs),
        };
        // Figma's font metrics often come out a few pixels wider than
        // Chromium's for the same string. A box Chromium sized tight will
        // then wrap a single line into two (e.g. "847TWh" → "847TW" + "h" on
        // the next row). For single-line runs with no hard break, pad the
        // box and mark noWrap so Figma never splits mid-word.
        //
        // Use range.getClientRects() (one rect per visual line) rather than
        // el.getClientRects() — on a block element the latter returns one
        // rect regardless of how many lines the text wraps to, which would
        // misclassify a wrapped paragraph as a single line.
        const range = document.createRange();
        range.selectNodeContents(el);
        const rangeRects = [...range.getClientRects()];
        const lineCount = countLines(rangeRects);
        if (lineCount === 1 && !hasHardBreak) {
          let widest = 0;
          for (const r of rangeRects) if (r.width > widest) widest = r.width;
          if (widest > 0) {
            const padded = Math.ceil(widest * 1.08) + 4;
            if (padded > el2.width) el2.width = padded;
          }
          el2.noWrap = true;
        }
        pushElement(el2);
      }

      function runStyle(cs) {
        const out = {};
        // A run can be in a different family from the paragraph around it —
        // a Space Grotesk unit suffix inside an Instrument Serif number, say.
        // Without this the run inherits the paragraph's family and is written
        // as a face that may not exist in it.
        if (cs.fontFamily) out.font = cs.fontFamily;
        const w = parseInt(cs.fontWeight, 10);
        if (Number.isFinite(w)) out.weight = w;
        if (cs.fontStyle === 'italic') out.style = 'italic';
        if (cs.color) out.color = cs.color;
        return out;
      }

      function maybeEmitShape(el, cs, rect) {
        if (rect.width <= 0 || rect.height <= 0) return;
        const bg = cs.backgroundColor;
        const hasBg = isVisibleColor(bg);
        const bgImage = cs.backgroundImage;
        let bgLayers = parseBackgroundImage(bgImage, warnings, el);
        const sides = ['top', 'right', 'bottom', 'left']
          .map((side) => ({
            side,
            width: pxFromCs(cs.getPropertyValue(`border-${side}-width`)),
            style: cs.getPropertyValue(`border-${side}-style`),
            color: cs.getPropertyValue(`border-${side}-color`),
          }))
          .filter((b) => b.width > 0 && b.style !== 'none' && isVisibleColor(b.color));

        if (!hasBg && !bgLayers.length && sides.length === 0) return;

        const radius = pxFromCs(cs.borderRadius);
        const minSide = Math.min(rect.width, rect.height);
        const maxSide = Math.max(rect.width, rect.height);
        const isEllipse =
          minSide > 0 &&
          minSide / maxSide >= 0.8 &&
          radius > 0 &&
          radius >= minSide / 2 - 0.5;

        // All four borders identical → emit a single stroked shape instead of
        // four axis-aligned side rects, which can't form a rounded outline.
        const uniformBorder = sides.length === 4
          && sides.every((b) => b.width === sides[0].width && b.color === sides[0].color)
          ? sides[0] : null;

        // Peel an alpha channel off an rgba() background and surface it as
        // element.opacity so api.mjs's parseColor (6-hex-only) still accepts
        // the fill. Skip hex inputs (already opaque).
        let emitFill = hasBg ? bg : undefined;
        let fillAlpha = 1;
        if (hasBg) {
          const m = /^rgba?\(\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)(?:\s*,\s*([\d.]+))?\s*\)$/i.exec(bg);
          if (m) {
            const r = Math.round(parseFloat(m[1]));
            const g = Math.round(parseFloat(m[2]));
            const b = Math.round(parseFloat(m[3]));
            fillAlpha = m[4] !== undefined ? parseFloat(m[4]) : 1;
            emitFill = '#' + [r, g, b].map((n) => n.toString(16).padStart(2, '0')).join('').toUpperCase();
          }
        }

        // A conic-gradient is how CSS draws a pie chart. Figma has no CSS-conic
        // equivalent that survives a paint round-trip, so emit the cone as real
        // wedge geometry and let the SVG path pipeline turn it into VECTOR
        // nodes — crisp edges at any zoom, and each slice stays selectable.
        const conicLayers = bgLayers.filter(l => l.kind === 'conic');
        if (conicLayers.length) {
          bgLayers = bgLayers.filter(l => l.kind !== 'conic');
          for (const layer of conicLayers) {
            const inline = conicGradientSvg(rect.width, rect.height, layer);
            if (!inline) continue;
            pushElement({
              type: 'svg',
              x: rect.x, y: rect.y, width: rect.width, height: rect.height,
              viewBox: `0 0 ${rect.width} ${rect.height}`,
              inline,
              ...(parseFloat(cs.opacity) < 1 ? { opacity: parseFloat(cs.opacity) } : {}),
            });
          }
        }

        const blurRadius = parseBlurFilter(cs.filter);
        if (hasBg || bgLayers.length || uniformBorder) {
          const e = {
            type: isEllipse ? 'ellipse' : 'rect',
            x: rect.x, y: rect.y, width: rect.width, height: rect.height,
          };
          if (hasBg) e.fill = emitFill;
          if (hasBg && fillAlpha < 1) e.opacity = fillAlpha;
          if (bgLayers.length) e.backgroundLayers = bgLayers;
          if (radius > 0 && !isEllipse) e.cornerRadius = radius;
          if (uniformBorder) {
            e.stroke = uniformBorder.color;
            e.strokeWidth = uniformBorder.width;
          }
          if (blurRadius != null) e.filter = { blur: blurRadius };
          pushElement(e);
        }
        if (!uniformBorder) {
          for (const b of sides) {
            let r;
            if (b.side === 'top') r = { type: 'rect', x: rect.x, y: rect.y, width: rect.width, height: b.width, fill: b.color };
            else if (b.side === 'bottom') r = { type: 'rect', x: rect.x, y: rect.y + rect.height - b.width, width: rect.width, height: b.width, fill: b.color };
            else if (b.side === 'left') r = { type: 'rect', x: rect.x, y: rect.y, width: b.width, height: rect.height, fill: b.color };
            else if (b.side === 'right') r = { type: 'rect', x: rect.x + rect.width - b.width, y: rect.y, width: b.width, height: rect.height, fill: b.color };
            if (r) pushElement(r);
          }
        }
      }

      function emitPseudo(host, hostCs, which, hostRect) {
        const cs = getComputedStyle(host, which);
        const content = cs.content;
        if (!content || content === 'none' || content === 'normal') return;

        let text = content;
        const q = text.match(/^(['"])([\s\S]*)\1$/);
        if (q) text = q[2];
        else if (/^(attr|counter|counters|url|var)\s*\(/.test(text)) {
          warnings.push({ msg: `pseudo content uses ${text.match(/^(\w+)/)[1]}() — not rendered`, sample: elPath(host) });
          return;
        }

        const left = pxFromCs(cs.left);
        const top = pxFromCs(cs.top);
        const right = pxFromCs(cs.right);
        const bottom = pxFromCs(cs.bottom);
        const w = pxFromCs(cs.width);
        const h = pxFromCs(cs.height);
        const pos = cs.position;

        let px = hostRect.x;
        let py = hostRect.y;
        if (pos === 'absolute') {
          if (left > 0 || cs.left !== 'auto') px = hostRect.x + left;
          else if (cs.right !== 'auto' && w > 0) px = hostRect.x + hostRect.width - right - w;
          if (top > 0 || cs.top !== 'auto') py = hostRect.y + top;
          else if (cs.bottom !== 'auto' && h > 0) py = hostRect.y + hostRect.height - bottom - h;
        }

        const fontSize = pxFromCs(cs.fontSize) || 16;
        const pw = w > 0 ? w : (text === '' ? 0 : Math.ceil(text.length * fontSize * 0.6));
        const ph = h > 0 ? h : (text === '' ? 0 : Math.ceil(fontSize * 1.2));

        if (text === '') {
          if (pw <= 0 || ph <= 0) return;
          const bg = cs.backgroundColor;
          if (!isVisibleColor(bg)) return;
          const minSide = Math.min(pw, ph);
          const maxSide = Math.max(pw, ph);
          const radius = pxFromCs(cs.borderRadius);
          const isEllipse =
            minSide / maxSide >= 0.8 &&
            radius >= minSide / 2 - 0.5;
          // Mark absolute-positioned ::before dots at the host's left edge as
          // "bullet markers" so a later pass can inset any host text whose box
          // collides with them. Source HTML sometimes drops a <p> outside its
          // <ul> — the author's CSS positions a • via ::before but forgets
          // padding-left on that paragraph, so the dot overlaps the first word.
          const isMarkerCandidate =
            which === '::before' &&
            pos === 'absolute' &&
            isEllipse &&
            maxSide <= 16 &&
            Math.abs(px - hostRect.x) <= 2;
          const node = {
            type: isEllipse ? 'ellipse' : 'rect',
            x: px, y: py, width: pw, height: ph,
            fill: bg,
          };
          if (isMarkerCandidate) node._leftMarker = true;
          pushElement(node);
          return;
        }

        const pstyle = textStyle(cs);
        const pel = {
          type: 'text',
          text,
          x: px, y: py, width: pw, height: ph,
          ...pstyle,
        };
        // Not routed through `alignmentIsLoadBearing`: this is ::before/::after
        // content, which no Range can select, so there is no text width to
        // compare the box against. Left as the conservative test rather than
        // guessing at a width — generated content is usually tight to its own
        // text, so if a wrap turns up here, measuring is the next step.
        if (!pstyle.align) pel.noWrap = true;
        const protation = verticalTextRotation(cs);
        if (protation) applyTextRotation(pel, protation);
        pushElement(pel);
      }

      // SVG constructs the handoff has no parser for. It reads path, rect,
      // circle, ellipse, line, polygon, polyline and text, plus both gradient
      // types — everything else in the markup is discarded on the way to the
      // deck. Discarding it quietly is the problem: the conversion reports
      // success while artwork goes missing, which is only noticed by opening
      // the deck and comparing it to the original by eye.
      function warnUnsupportedSvg(markup, el) {
        // Declared inside: the SVG emission sites run before this point in the
        // enclosing scope, so a `const` out here is still in its temporal dead
        // zone when they call in.
        // `mask` and `filter` are deliberately absent from the converter, not
        // merely unimplemented: feColorMatrix and its relatives have no Figma
        // equivalent short of baking the affected content to a bitmap, which
        // trades the editable vectors the converter exists to produce.
        //
        // `pattern` and `clipPath` used to be on this list and are not any
        // more: the handoff converts them. What it *cannot* convert — a clip
        // that is not a rectangle, a pattern with no axis-aligned tile — is
        // reported from there, where the parsed geometry says so. Answering it
        // twice, once from the markup here and once from the geometry there,
        // is how the two halves of a pipeline start disagreeing.
        const UNSUPPORTED_SVG = {
          mask: 'masks',
          filter: 'filter effects',
          foreignObject: 'foreignObject content',
        };
        for (const [tag, label] of Object.entries(UNSUPPORTED_SVG)) {
          const re = new RegExp('<' + tag + '[\\s>/]', 'i');
          if (re.test(markup)) {
            warnings.push({ msg: `SVG ${label} are not converted and were dropped`, sample: elPath(el) });
          }
        }
        // <use>/<symbol> are resolved by the handoff (expandSvgUse inlines the
        // referenced geometry), so their presence is no longer a loss and
        // warning about it would be noise. A reference that cannot be resolved
        // still is one: the copy is never made, so that artwork is absent from
        // the deck and nothing else in the pipeline would say so.
        const useRe = /<use\b[^>]*>/gi;
        let um;
        while ((um = useRe.exec(markup)) !== null) {
          // The quote characters are written as \x22 and \x27, not literally.
          // The portability scan strips strings with a regex that knows
          // nothing about regex literals, so a `['"]` class reads to it as a
          // single-quoted string opening — which shifts every quote after it
          // by one and eventually leaves the file looking as though it ends
          // inside a string. The scan then backtracks over the whole file and
          // never returns: `npx vitest run test/core/core-portability` hangs
          // rather than fails, which is a much worse thing to debug than a
          // wrong answer. Escapes keep the class quote-free and match the same
          // two characters. Back-referenced so it accepts either spelling.
          const hrefMatch = um[0].match(/\b(?:xlink:)?href\s*=\s*([\x22\x27])(.*?)\1/i);
          const href = hrefMatch ? hrefMatch[2] : null;
          if (!href || href[0] !== '#') {
            // An external reference would mean fetching a second document, and
            // nothing from the export may leave the machine.
            warnings.push({ msg: `SVG <use> without a local #id reference was skipped`, sample: elPath(el) });
            continue;
          }
          const id = href.slice(1).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          if (!new RegExp('\\bid\\s*=\\s*"' + id + '"').test(markup)) {
            warnings.push({ msg: `SVG <use href="${href}"> references an id that is not in the markup — skipped`, sample: elPath(el) });
          }
        }
        // <image> is off the unsupported list for the same reason <use> is:
        // the handoff converts it now, so listing it would train people to
        // ignore a list that exists to be believed. What is still a loss is a
        // source we refuse to read. A `data:` URI carries its own bytes and a
        // `blob:` URL is the export's own, already paired back to a decoded
        // asset before the handoff sees it; every other scheme would have to
        // be fetched, and nothing from an export leaves the machine.
        const imageRe = /<image\b[^>]*>/gi;
        let im;
        while ((im = imageRe.exec(markup)) !== null) {
          const hrefMatch = im[0].match(/\b(?:xlink:)?href\s*=\s*([\x22\x27])(.*?)\1/i);
          const href = hrefMatch ? hrefMatch[2] : null;
          if (!href) {
            warnings.push({ msg: `SVG <image> without an href was skipped`, sample: elPath(el) });
            continue;
          }
          if (/^(?:data:|blob:)/i.test(href)) continue;
          // Truncated: an external URL can be arbitrarily long, and the point
          // of the line is which slide to look at, not the whole address.
          warnings.push({
            msg: `SVG <image> with an external source was skipped — the converter never fetches from an export (${href.slice(0, 60)})`,
            sample: elPath(el),
          });
        }
      }

      // A function declaration, not a const: the emission sites below run
      // before this point in the enclosing scope, and a `const` would still be
      // in its temporal dead zone when they call in. Declarations hoist.
      //
      // CSS and Figma name the same blend operations, so this is a rename
      // rather than an approximation. `plus-lighter` and `plus-darker` have no
      // Figma counterpart and return undefined, which is what earns a warning.
      function figmaBlendMode(css) {
        switch (css) {
          case 'multiply': return 'MULTIPLY';
          case 'screen': return 'SCREEN';
          case 'overlay': return 'OVERLAY';
          case 'darken': return 'DARKEN';
          case 'lighten': return 'LIGHTEN';
          case 'color-dodge': return 'COLOR_DODGE';
          case 'color-burn': return 'COLOR_BURN';
          case 'hard-light': return 'HARD_LIGHT';
          case 'soft-light': return 'SOFT_LIGHT';
          case 'difference': return 'DIFFERENCE';
          case 'exclusion': return 'EXCLUSION';
          case 'hue': return 'HUE';
          case 'saturation': return 'SATURATION';
          case 'color': return 'COLOR';
          case 'luminosity': return 'LUMINOSITY';
          default: return undefined;
        }
      }

      function collectWarnings(el, cs) {
        if (cs.transform && cs.transform !== 'none') {
          // getBoundingClientRect() already reflects CSS transforms, so a pure
          // translation is effectively already baked into the rect we
          // extracted. Only warn for rotate/scale/skew, which we can't yet
          // represent in the deck.
          if (!isPureTranslateTransform(cs.transform) && !verticalTextRotation(cs)) {
            warnings.push({ msg: `transform:${cs.transform} (non-translate) ignored`, sample: elPath(el) });
          }
        }
        if (cs.clipPath && cs.clipPath !== 'none') {
          warnings.push({ msg: `clip-path ignored`, sample: elPath(el) });
        }
        if (cs.filter && cs.filter !== 'none') {
          // blur() is now supported (mapped to Figma FOREGROUND_BLUR at
          // dispatch time). Every other filter value still warns.
          if (parseBlurFilter(cs.filter) == null) {
            warnings.push({ msg: `filter:${cs.filter} ignored`, sample: elPath(el) });
          }
        }
        // Figma's blend modes are the same set CSS has, so most of these
        // convert rather than being dropped. Only the ones with no counterpart
        // are worth a warning.
        if (cs.mixBlendMode && cs.mixBlendMode !== 'normal' && !figmaBlendMode(cs.mixBlendMode)) {
          warnings.push({ msg: `mix-blend-mode:${cs.mixBlendMode} ignored`, sample: elPath(el) });
        }
      }

      function textStyle(cs) {
        const size = pxFromCs(cs.fontSize);
        const lh = parseLineHeight(cs.lineHeight, size);
        const out = {
          font: cs.fontFamily,
          size,
          weight: parseInt(cs.fontWeight, 10) || undefined,
        };
        if (cs.fontStyle === 'italic') out.style = 'italic';
        if (cs.color) out.color = cs.color;
        const ls = pxFromCs(cs.letterSpacing);
        if (ls) out.letterSpacing = ls;
        if (lh != null) out.lineHeight = lh;
        const al = cs.textAlign;
        if (al && al !== 'start' && al !== 'left' && al !== 'auto') out.align = al;
        // CSS vertical-align on table cells / flex / inline-block maps to
        // Figma's textAlignVertical. Only emit for the block-valign values
        // Figma supports; baseline/sub/super are inline-box adjustments that
        // don't translate to Figma's text alignment.
        const va = cs.verticalAlign;
        if (va === 'middle') out.verticalAlign = 'middle';
        else if (va === 'bottom') out.verticalAlign = 'bottom';
        const op = parseFloat(cs.opacity);
        if (!Number.isNaN(op) && op < 1) out.opacity = op;
        return out;
      }

      function parseLineHeight(v, fontSize) {
        if (!v || v === 'normal') return undefined;
        const n = pxFromCs(v);
        return Number.isFinite(n) ? n : undefined;
      }

      // True iff the computed transform is a half turn — `rotate(180deg)` and
      // its `matrix(-1, 0, 0, -1, …)` computed form.
      function isHalfTurnTransform(tr) {
        const m = String(tr || '').match(/^matrix\s*\(\s*([^)]+)\)\s*$/);
        if (!m) return false;
        const p = m[1].split(',').map((v) => parseFloat(v.trim()));
        if (p.length !== 6 || p.some((v) => !Number.isFinite(v))) return false;
        const [a, b, c, d] = p;
        return (
          Math.abs(a + 1) < 1e-3 && Math.abs(d + 1) < 1e-3 &&
          Math.abs(b) < 1e-3 && Math.abs(c) < 1e-3
        );
      }

      /**
       * A CSS vertical writing mode, expressed as a Figma node rotation in
       * degrees clockwise, or null for ordinary horizontal text.
       *
       * `writing-mode: vertical-rl` runs the glyphs top-to-bottom — the same
       * picture as horizontal text turned a quarter turn clockwise. Pairing it
       * with `transform: rotate(180deg)` is the standard idiom for a side
       * label that reads bottom-to-top, which is the anticlockwise quarter
       * turn. `sideways-lr` reads bottom-to-top on its own.
       *
       * Figma has no writing mode, but it does have node rotation, and for a
       * single run the two produce the same glyphs in the same places. Without
       * this the label was laid out horizontally inside its tall narrow box,
       * so a four-label band down the edge of a slide came out as four
       * squashed stacks.
       */
      function verticalTextRotation(cs) {
        const wm = cs.writingMode;
        if (!wm || wm === 'horizontal-tb') return null;
        const base = wm === 'sideways-lr' ? -90 : 90;
        return isHalfTurnTransform(cs.transform) ? -base : base;
      }

      /**
       * Re-express an already-positioned text element as an unrotated run plus
       * a rotation.
       *
       * The rect came from `getBoundingClientRect`, so it is the on-screen box:
       * its long side is the length of the text and its short side the line
       * box. The node's own box is that transposed. Figma rotates a node about
       * its top-left corner, so the origin also has to move to whichever corner
       * lands on the screen box's top-left once turned.
       */
      function applyTextRotation(el2, deg) {
        const { x, y, width, height } = el2;
        el2.width = height;
        el2.height = width;
        el2.rotate = deg;
        if (deg < 0) { el2.x = x; el2.y = y + height; }
        else { el2.x = x + width; el2.y = y; }
        // The box is now exactly the measured run; re-wrapping it would only
        // ever break the label across the short axis.
        el2.noWrap = true;
      }

      // True iff the computed `transform` value is equivalent to a pure
      // 2D translation. getBoundingClientRect already includes it.
      function isPureTranslateTransform(tr) {
        if (!tr || tr === 'none') return true;
        const s = String(tr).trim();
        if (/^translate(X|Y|3d)?\s*\(/.test(s)) {
          if (/^translate3d\s*\(/.test(s)) {
            const parts = s.replace(/^translate3d\s*\(/, '').replace(/\)\s*$/, '').split(',');
            const z = parts[2] ? parseFloat(parts[2]) : 0;
            return Math.abs(z) < 1e-6;
          }
          return true;
        }
        const m2 = s.match(/^matrix\s*\(\s*([^)]+)\)\s*$/);
        if (m2) {
          const p = m2[1].split(',').map((v) => parseFloat(v.trim()));
          if (p.length !== 6 || p.some((v) => !Number.isFinite(v))) return false;
          const [a, b, c, d] = p;
          return (
            Math.abs(a - 1) < 1e-6 &&
            Math.abs(b) < 1e-6 &&
            Math.abs(c) < 1e-6 &&
            Math.abs(d - 1) < 1e-6
          );
        }
        const m3 = s.match(/^matrix3d\s*\(\s*([^)]+)\)\s*$/);
        if (m3) {
          const p = m3[1].split(',').map((v) => parseFloat(v.trim()));
          if (p.length !== 16 || p.some((v) => !Number.isFinite(v))) return false;
          const expectOne = [0, 5, 10, 15];
          for (let i = 0; i < 16; i++) {
            if (i === 12 || i === 13 || i === 14) continue;
            const want = expectOne.includes(i) ? 1 : 0;
            if (Math.abs(p[i] - want) >= 1e-6) return false;
          }
          return true;
        }
        return false;
      }

      // Parse `cs.backgroundImage` into layer descriptors, CSS painting order
      // (first = topmost). Recognised: linear-gradient, radial-gradient.
      // url(...) is silently accepted but not rendered (falls back to bg-color).
      function parseBackgroundImage(bgImage, warnings, el) {
        if (!bgImage || bgImage === 'none') return [];
        const layers = splitTopLevel(bgImage, ',');
        const out = [];
        for (const raw of layers) {
          const layer = raw.trim();
          if (!layer || layer === 'none') continue;
          const headMatch = /^([a-z-]+)\s*\(([\s\S]*)\)\s*$/i.exec(layer);
          if (!headMatch) {
            warnings?.push({ msg: `background-image layer not a gradient/url — ignored`, sample: elPath(el) });
            continue;
          }
          const kind = headMatch[1].toLowerCase();
          const inner = headMatch[2];
          if (kind === 'linear-gradient') {
            const g = parseCssLinearGradient(inner);
            if (g) out.push(g);
          } else if (kind === 'radial-gradient') {
            const g = parseCssRadialGradient(inner);
            if (g) out.push(g);
          } else if (kind === 'repeating-linear-gradient') {
            const g = parseCssRepeatingLinearGradient(inner, el, warnings);
            if (g) out.push(g);
            else warnings?.push({ msg: 'repeating-linear-gradient could not be expanded — ignored', sample: elPath(el) });
          } else if (kind === 'conic-gradient') {
            const g = parseCssConicGradient(inner);
            if (g) out.push(g);
            else warnings?.push({ msg: 'conic-gradient could not be parsed — ignored', sample: elPath(el) });
          } else if (kind === 'url') {
            continue;
          } else {
            warnings?.push({ msg: `unsupported background-image "${kind}" — ignored`, sample: elPath(el) });
          }
        }
        return out;
      }

      function parseLinearMaskImage(cs, el) {
        const raw = (cs.maskImage && cs.maskImage !== 'none')
          ? cs.maskImage
          : (cs.webkitMaskImage && cs.webkitMaskImage !== 'none' ? cs.webkitMaskImage : null);
        if (!raw) return null;

        const layers = parseBackgroundImage(raw, null, el);
        if (layers.length === 1 && layers[0].kind === 'linear') return layers[0];

        warnings.push({
          msg: 'mask-image is not one linear gradient — ignored',
          sample: elPath(el),
        });
        return null;
      }

      function splitTopLevel(str, sep) {
        const out = [];
        let depth = 0;
        let start = 0;
        for (let i = 0; i < str.length; i++) {
          const c = str[i];
          if (c === '(') depth++;
          else if (c === ')') depth = Math.max(0, depth - 1);
          else if (c === sep && depth === 0) {
            out.push(str.slice(start, i));
            start = i + 1;
          }
        }
        out.push(str.slice(start));
        return out;
      }


      // ── repeating-linear-gradient ──────────────────────────────────────
      //
      // Figma has no repeating gradient, so the repetition is expanded into
      // explicit stops. Two things make this more than a rename of
      // `linear-gradient`:
      //
      //   * stop positions are usually in px, which only mean something once
      //     you know how long the gradient line is across this element;
      //   * the double-position shorthand (`red 0 60px`) is one token that
      //     declares two stops, and it is how hard-edged stripes are written.
      //
      // The gradient line for angle t across a w x h box is
      // |w*sin t| + |h*cos t| (CSS Images 3). One period is the last stop's
      // offset; it tiles until the line is covered.
      function gradientLineLength(angleDeg, w, h) {
        const t = (angleDeg * Math.PI) / 180;
        return Math.abs(w * Math.sin(t)) + Math.abs(h * Math.cos(t));
      }

      // `red 0 60px` -> two stops. Lengths stay in px here; they are divided
      // by the line length once it is known.
      function parseRepeatingStops(parts) {
        const out = [];
        for (const part of parts) {
          const color = extractLeadingColor(part);
          if (!color) return null;
          const rest = part.slice(color.length).trim();
          if (!rest) { out.push({ color, px: null }); continue; }
          for (const tok of rest.split(/\s+/).filter(Boolean)) {
            const m = /^(-?[\d.]+)(px|%)?$/.exec(tok);
            if (!m) return null;
            out.push({ color, px: m[2] === '%' ? { pct: parseFloat(m[1]) / 100 } : parseFloat(m[1]) });
          }
        }
        return out.length ? out : null;
      }

      function parseCssRepeatingLinearGradient(body, el, warnings) {
        // Declared here rather than at module scope: the extractor payload is
        // one function body, and the element walk runs before a top-level
        // `const` on this line would have initialised.
        // Generous: a stop is a colour plus a float, and Figma handles long
        // stop lists without complaint. The budget exists to stop a hairline
        // hatch over a full slide from producing tens of thousands, not to
        // keep the count tidy.
        const REPEAT_STOP_BUDGET = 4096;
        const parts = splitTopLevel(body, ',').map((x) => x.trim());
        if (!parts.length) return null;

        let angleDeg = 180;
        const angle = /^(-?[\d.]+)deg$/i.exec(parts[0]);
        const to = /^to\s+(.+)$/i.exec(parts[0]);
        if (angle) { angleDeg = parseFloat(angle[1]); parts.shift(); }
        else if (to) { angleDeg = sideToAngleDeg(to[1].trim().toLowerCase()); parts.shift(); }

        const raw = parseRepeatingStops(parts);
        if (!raw) return null;

        const rect = el.getBoundingClientRect();
        const line = gradientLineLength(angleDeg, rect.width, rect.height);
        if (!(line > 0)) return null;

        // Resolve every offset to px along the line, filling gaps evenly.
        const stops = raw.map((st) => ({
          color: st.color,
          at: st.px === null ? null : (typeof st.px === 'object' ? st.px.pct * line : st.px),
        }));
        if (stops[0].at === null) stops[0].at = 0;
        for (let i = 1; i < stops.length; i++) {
          if (stops[i].at !== null) continue;
          let j = i;
          while (j < stops.length && stops[j].at === null) j++;
          const from = stops[i - 1].at;
          const to2 = j < stops.length ? stops[j].at : line;
          for (let k = 0; k < j - i; k++) stops[i + k].at = from + ((to2 - from) * (k + 1)) / (j - i + 1);
          i = j - 1;
        }

        const period = stops[stops.length - 1].at;
        if (!(period > 0)) return null;

        const periods = Math.ceil(line / period);
        if (periods * stops.length > REPEAT_STOP_BUDGET) {
          // A fine hatch over a large box runs to thousands of stops. Emitting
          // them would bloat the deck for a texture nobody can resolve; say so
          // rather than silently drawing one giant stripe.
          warnings?.push({
            msg: `repeating-linear-gradient needs ${periods * stops.length} stops to expand `
               + `(budget ${REPEAT_STOP_BUDGET}) — too fine to reproduce, so it is omitted`,
            sample: elPath(el),
          });
          return null;
        }

        const expanded = [];
        for (let n = 0; n < periods; n++) {
          for (const st of stops) {
            const pos = (n * period + st.at) / line;
            if (pos > 1.0001) break;
            expanded.push({ color: st.color, pos: Math.max(0, Math.min(1, pos)) });
          }
        }
        if (expanded.length < 2) return null;
        if (expanded[expanded.length - 1].pos < 1) {
          expanded.push({ color: expanded[expanded.length - 1].color, pos: 1 });
        }
        return { kind: 'linear', angleDeg, stops: expanded };
      }

      function parseCssLinearGradient(body) {
        const parts = splitTopLevel(body, ',').map(s => s.trim());
        if (parts.length === 0) return null;
        let angleDeg = 180;
        let first = parts[0];
        const angleMatch = /^(-?\d+(?:\.\d+)?)deg$/i.exec(first);
        const toMatch = /^to\s+(.+)$/i.exec(first);
        if (angleMatch) {
          angleDeg = parseFloat(angleMatch[1]);
          parts.shift();
        } else if (toMatch) {
          angleDeg = sideToAngleDeg(toMatch[1].trim().toLowerCase());
          parts.shift();
        } else if (/^(-?\d+(?:\.\d+)?)(rad|grad|turn)$/i.test(first)) {
          const um = /^(-?\d+(?:\.\d+)?)(rad|grad|turn)$/i.exec(first);
          const v = parseFloat(um[1]);
          const u = um[2].toLowerCase();
          if (u === 'rad') angleDeg = v * 180 / Math.PI;
          else if (u === 'grad') angleDeg = v * 0.9;
          else if (u === 'turn') angleDeg = v * 360;
          parts.shift();
        }
        const stops = parseColorStops(parts);
        if (!stops.length) return null;
        return { kind: 'linear', angleDeg, stops };
      }

      function sideToAngleDeg(side) {
        switch (side) {
          case 'top': return 0;
          case 'right': return 90;
          case 'bottom': return 180;
          case 'left': return 270;
          case 'top right': case 'right top': return 45;
          case 'bottom right': case 'right bottom': return 135;
          case 'bottom left': case 'left bottom': return 225;
          case 'top left': case 'left top': return 315;
          default: return 180;
        }
      }

      // conic-gradient stop positions may be angles as well as percentages
      // (`conic-gradient(#123 0deg, #123 77%, ...)`), which the shared
      // percentage-only parser rejects.
      function parseAnglePositionValue(tok) {
        const m = /^(-?\d*\.?\d+)(%|deg|turn|rad|grad)?$/i.exec(tok);
        if (!m) return null;
        const v = parseFloat(m[1]);
        const u = (m[2] || '%').toLowerCase();
        if (u === '%') return v / 100;
        if (u === 'deg') return v / 360;
        if (u === 'turn') return v;
        if (u === 'rad') return v / (2 * Math.PI);
        if (u === 'grad') return v / 400;
        return null;
      }

      function parseAngleColorStops(parts) {
        const raw = [];
        for (const p of parts) {
          const color = extractLeadingColor(p);
          if (!color) continue;
          const rest = p.slice(color.length).trim();
          const positions = [];
          if (rest) {
            for (const t of rest.split(/\s+/).filter(Boolean)) {
              const v = parseAnglePositionValue(t);
              if (v != null) positions.push(v);
            }
          }
          if (positions.length === 0) raw.push({ color, pos: null });
          else for (const pos of positions) raw.push({ color, pos });
        }
        if (!raw.length) return [];
        if (raw[0].pos == null) raw[0].pos = 0;
        if (raw[raw.length - 1].pos == null) raw[raw.length - 1].pos = 1;
        for (let i = 0; i < raw.length; i++) {
          if (raw[i].pos != null) continue;
          let j = i;
          while (j < raw.length && raw[j].pos == null) j++;
          const startPos = raw[i - 1].pos;
          const endPos = raw[j]?.pos ?? 1;
          const gap = j - i + 1;
          for (let k = 0; k < j - i; k++) {
            raw[i + k].pos = startPos + (endPos - startPos) * (k + 1) / gap;
          }
          i = j - 1;
        }
        return raw.map(s => ({ color: s.color, pos: Math.max(0, Math.min(1, s.pos)) }));
      }

      // Render a parsed conic gradient as pie wedges. CSS starts the cone at
      // 12 o'clock and sweeps clockwise, hence the -90deg screen rotation.
      // Constant-colour spans (the hard stops that make a pie chart) become a
      // single exact wedge; genuine colour ramps are subdivided.
      function conicGradientSvg(w, h, layer) {
        if (!(w > 0) || !(h > 0)) return null;
        const cx = w / 2, cy = h / 2, r = Math.min(w, h) / 2;
        const at = (deg) => {
          const t = (deg - 90) * Math.PI / 180;
          return [cx + r * Math.cos(t), cy + r * Math.sin(t)];
        };
        const rgb = (c) => {
          const m = /rgba?\(([^)]+)\)/i.exec(c);
          if (!m) return null;
          const n = m[1].split(',').map(s => parseFloat(s.trim()));
          return [n[0], n[1], n[2]];
        };
        const mix = (c0, c1, t) => {
          const a = rgb(c0), b = rgb(c1);
          if (!a || !b) return c0;
          const v = a.map((x, i) => Math.round(x + (b[i] - x) * t));
          return `rgb(${v[0]}, ${v[1]}, ${v[2]})`;
        };
        const wedge = (a0, a1, color) => {
          const sweep = a1 - a0;
          if (sweep <= 0.0001) return '';
          if (sweep >= 359.999) {
            return `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${color}"/>`;
          }
          const [x0, y0] = at(a0), [x1, y1] = at(a1);
          const large = sweep > 180 ? 1 : 0;
          return `<path d="M ${cx.toFixed(3)} ${cy.toFixed(3)} L ${x0.toFixed(3)} ${y0.toFixed(3)}`
            + ` A ${r.toFixed(3)} ${r.toFixed(3)} 0 ${large} 1 ${x1.toFixed(3)} ${y1.toFixed(3)} Z" fill="${color}"/>`;
        };
        const parts = [];
        for (let i = 0; i < layer.stops.length - 1; i++) {
          const a = layer.stops[i], b = layer.stops[i + 1];
          const a0 = layer.fromDeg + a.pos * 360;
          const a1 = layer.fromDeg + b.pos * 360;
          if (a1 - a0 <= 0.0001) continue;
          if (a.color === b.color) { parts.push(wedge(a0, a1, a.color)); continue; }
          const steps = Math.max(1, Math.min(48, Math.ceil((a1 - a0) / 6)));
          for (let k = 0; k < steps; k++) {
            parts.push(wedge(
              a0 + (a1 - a0) * k / steps,
              a0 + (a1 - a0) * (k + 1) / steps,
              mix(a.color, b.color, (k + 0.5) / steps),
            ));
          }
        }
        const body = parts.filter(Boolean).join('');
        if (!body) return null;
        return `<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg">${body}</svg>`;
      }

      function parseCssConicGradient(body) {
        const parts = splitTopLevel(body, ',').map(s => s.trim());
        if (!parts.length) return null;
        let fromDeg = 0;
        const fromMatch = /^from\s+(-?\d*\.?\d+)(deg|turn|rad|grad)?$/i.exec(parts[0]);
        if (fromMatch) {
          const v = parseFloat(fromMatch[1]);
          const u = (fromMatch[2] || 'deg').toLowerCase();
          fromDeg = u === 'deg' ? v : u === 'turn' ? v * 360 : u === 'rad' ? v * 180 / Math.PI : v * 0.9;
          parts.shift();
        }
        // `at <position>` re-centres the cone; we always centre on the box, so
        // consume it rather than mistaking it for a colour stop.
        if (parts.length && /^at\s+/i.test(parts[0])) parts.shift();
        const stops = parseAngleColorStops(parts);
        if (stops.length < 2) return null;
        return { kind: 'conic', fromDeg, stops };
      }

      function parseCssRadialGradient(body) {
        const parts = splitTopLevel(body, ',').map(s => s.trim());
        if (parts.length === 0) return null;
        let cx = 0.5, cy = 0.5, rx = 0.5, ry = 0.5;
        let stops;
        const head = parts[0];
        const looksLikeShape = /^(circle|ellipse)\b/i.test(head)
          || /\bat\b/i.test(head)
          || /^\d/.test(head)
          || /^-?\d*\.?\d+%/.test(head);
        if (looksLikeShape) {
          const shapePart = parts.shift();
          const atIdx = shapePart.search(/\bat\b/i);
          const sizeStr = atIdx >= 0 ? shapePart.slice(0, atIdx).trim() : shapePart.trim();
          const posStr = atIdx >= 0 ? shapePart.slice(atIdx + 2).trim() : '';
          const sizeTokens = sizeStr.split(/\s+/).filter(Boolean).filter(t => !/^(ellipse|circle)$/i.test(t));
          if (sizeTokens.length >= 2) {
            rx = cssLengthToUnit(sizeTokens[0], 'x');
            ry = cssLengthToUnit(sizeTokens[1], 'y');
          } else if (sizeTokens.length === 1) {
            const v = cssLengthToUnit(sizeTokens[0], 'x');
            rx = ry = v;
          }
          if (posStr) {
            const pos = parsePosition(posStr);
            cx = pos.x;
            cy = pos.y;
          }
        }
        stops = parseColorStops(parts);
        if (!stops.length) return null;
        if (!(rx > 0) || !(ry > 0)) return null;
        return { kind: 'radial', cx, cy, rx, ry, stops };
      }

      function cssLengthToUnit(tok, axis) {
        const m = /^(-?\d*\.?\d+)(%|px)?$/.exec(tok);
        if (!m) return 0.5;
        const v = parseFloat(m[1]);
        const u = m[2] || 'px';
        if (u === '%') return v / 100;
        return 0.5;
      }

      function parsePosition(str) {
        const toks = str.split(/\s+/).filter(Boolean);
        const map = { left: 0, center: 0.5, right: 1, top: 0, bottom: 1 };
        if (toks.length === 1) {
          const t = toks[0].toLowerCase();
          if (t in map) {
            const v = map[t];
            if (t === 'left' || t === 'right') return { x: v, y: 0.5 };
            if (t === 'top' || t === 'bottom') return { x: 0.5, y: v };
            return { x: 0.5, y: 0.5 };
          }
          const v = cssLengthToUnit(toks[0], 'x');
          return { x: v, y: 0.5 };
        }
        const x = toks[0] in map ? map[toks[0]] : cssLengthToUnit(toks[0], 'x');
        const y = toks[1] in map ? map[toks[1]] : cssLengthToUnit(toks[1], 'y');
        return { x, y };
      }

      function parseColorStops(parts) {
        const raw = [];
        for (const p of parts) {
          const color = extractLeadingColor(p);
          if (!color) continue;
          const rest = p.slice(color.length).trim();
          const positions = [];
          if (rest) {
            const tokens = rest.split(/\s+/).filter(Boolean);
            for (const t of tokens) {
              const v = parsePositionValue(t);
              if (v != null) positions.push(v);
            }
          }
          if (positions.length === 0) {
            raw.push({ color, pos: null });
          } else {
            for (const pos of positions) raw.push({ color, pos });
          }
        }
        if (raw.length === 0) return [];
        if (raw[0].pos == null) raw[0].pos = 0;
        if (raw[raw.length - 1].pos == null) raw[raw.length - 1].pos = 1;
        for (let i = 0; i < raw.length; i++) {
          if (raw[i].pos == null) {
            let j = i;
            while (j < raw.length && raw[j].pos == null) j++;
            const startPos = raw[i - 1].pos;
            const endPos = raw[j].pos ?? 1;
            const gap = j - i + 1;
            for (let k = 0; k < j - i; k++) {
              raw[i + k].pos = startPos + (endPos - startPos) * (k + 1) / gap;
            }
            i = j - 1;
          }
        }
        return raw.map(s => ({ color: s.color, pos: Math.max(0, Math.min(1, s.pos)) }));
      }

      function parsePositionValue(tok) {
        const m = /^(-?\d*\.?\d+)(%|px)?$/.exec(tok);
        if (!m) return null;
        const v = parseFloat(m[1]);
        const u = m[2] || '%';
        if (u === '%') return v / 100;
        return null;
      }

      function extractLeadingColor(str) {
        const s = str.trim();
        if (s.startsWith('#')) {
          const m = /^#[0-9a-fA-F]+/.exec(s);
          return m ? m[0] : null;
        }
        const fn = /^([a-zA-Z]+)\s*\(/.exec(s);
        if (fn) {
          let depth = 0;
          for (let i = 0; i < s.length; i++) {
            if (s[i] === '(') depth++;
            else if (s[i] === ')') {
              depth--;
              if (depth === 0) return s.slice(0, i + 1);
            }
          }
          return null;
        }
        const name = /^[a-zA-Z]+/.exec(s);
        return name ? name[0] : null;
      }

      // Parse `filter: blur(Npx)`. Returns the radius as a number, or null
      // if the filter value isn't a single blur(...) token. Multi-filter
      // strings like `blur(2px) drop-shadow(...)` return null because we
      // can't represent the composite in Figma yet.
      function parseBlurFilter(v) {
        if (!v || v === 'none') return null;
        const s = String(v).trim();
        const m = /^blur\(\s*(-?\d*\.?\d+)(?:px)?\s*\)$/i.exec(s);
        if (!m) return null;
        const n = parseFloat(m[1]);
        return Number.isFinite(n) && n > 0 ? n : null;
      }

      function isVisibleColor(c) {
        if (!c) return false;
        if (c === 'transparent') return false;
        if (c === 'rgba(0, 0, 0, 0)') return false;
        return true;
      }

      function pxFromCs(v) {
        if (v == null) return 0;
        const m = String(v).match(/(-?[\d.]+)/);
        return m ? parseFloat(m[1]) : 0;
      }

      function collapseText(s) {
        return String(s || '').replace(/\s+/g, ' ').trim();
      }

      function collapseTextPreservingBreaks(s) {
        const lines = String(s || '')
          .replace(/\r\n?/g, '\n')
          .split('\n')
          .map((line) => line.replace(/\s+/g, ' ').trim());
        while (lines.length > 0 && !lines[0]) lines.shift();
        while (lines.length > 0 && !lines[lines.length - 1]) lines.pop();
        return lines.join('\n');
      }

      function elPath(el) {
        const t = (el.tagName || '').toLowerCase();
        const id = el.getAttribute?.('id');
        const cls = el.className && typeof el.className === 'string' ? el.className.split(/\s+/).slice(0, 2).join('.') : '';
        let s = t;
        if (id) s += '#' + id;
        else if (cls) s += '.' + cls;
        return s;
      }
    }
  }, { flexAutoLayout });
}
