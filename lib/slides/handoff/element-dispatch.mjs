import { normalizeFont, NAMEABLE_WEIGHTS } from '../font-normalize.mjs';
import {
  sourceColorRisk,
  sourceDarkColorRisk,
} from '../../core/image-analysis.mjs';
 ...
 *
 * The endpoints come from `NAMEABLE_WEIGHTS` so the measurement clamp and the
 * handoff vocabulary cannot drift apart.
 */
function namedStyleWeight(weight) {
  const heavy = NAMEABLE_WEIGHTS[NAMEABLE_WEIGHTS.length - 1];
  const light = NAMEABLE_WEIGHTS[0];
  return typeof weight === 'number' && weight >= 600 ? heavy : light;
}
...
function parseObjectPosition(value) {
  ...
  const read = (part) => {
    if (Object.hasOwn(keywords, part)) return keywords[part];
    const pct = /^(-?[\d.]+)%$/.exec(part);
    return pct ? parseFloat(pct[1]) / 100 : null;
  };
...
// Figma's CROP transform maps the node's normalized coordinates into the
// normalized source-image window. CSS object-position chooses where the
// cover overflow is consumed, so its percentages directly place that window
// inside the source.
function objectPositionCropTransform(el, paint) {
  if (el.objectFit !== 'cover') return null;
  const pos = parseObjectPosition(el.objectPosition);
  if (!pos) return null;
  if (Math.abs(pos.x - 0.5) < 1e-9 && Math.abs(pos.y - 0.5) < 1e-9) return null;

  const sourceWidth = paint.originalImageWidth;
  const sourceHeight = paint.originalImageHeight;
  if (!(sourceWidth > 0) || !(sourceHeight > 0) || !(el.width > 0) || !(el.height > 0)) {
    return null;
  }
  ...
  let sx = 1;
  let sy = 1;
  if (sourceAspect > boxAspect) sx = boxAspect / sourceAspect;
  else if (sourceAspect < boxAspect) sy = sourceAspect / boxAspect;

  return {
    m00: sx, m01: 0, m02: (1 - sx) * pos.x,
    m10: 0, m11: sy, m12: (1 - sy) * pos.y,
  };
}
...
// Bake only the two legacy mask-style filters that have no native Color Adjust
// equivalent: invert(1) and brightness(0) invert(1). Ordinary photographic
// adjustments stay on the original IMAGE paint through `paintFilter`.
//
...
//
// Both implementations preserve alpha and return lossless PNG, so the authored
// transform is not followed by an unrelated lossy compression pass.
//
// Figma's exposure control is a tone curve, not a gain.
//
...
/**
 * Figma's contrast control is far weaker than it looks.
 *
...
/**
 * Extra native tone controls that compensate for Exposure's highlight rolloff.
 *
 * CSS brightness is a linear gain followed by clipping. Exposure alone is not:
 * it compresses a photograph's histogram by 16.9% more than CSS at brightness
 * 1.18, and by 30.8% more at 1.55. Native Highlights and Shadows recover much
 * of the missing transfer-curve spread while keeping the original image
 * editable.
 *
 * The 1.18 anchor is the mild brightness-only fit; the 1.55 anchor is the best
 * complete-chain fit with the fixture's strong contrast. Each anchor is:
 *   [CSS brightness, correction to exposureForBrightness, highlights, shadows]
 *
 * The correction is deliberately expressed as a small delta from the full
 * Exposure curve. Above the strongest measured anchor the extra controls and
 * delta clamp, while Exposure itself can continue through its calibrated
 * range.
 */
export const BRIGHTNESS_TONE_REFINEMENT = [
  [1, 0, 0, 0],
  [1.18, -0.0034, 0.55, -0.15],
  [1.55, 0, 1, -0.75],
];

function brightnessToneRefinement(multiplier) {
  const first = BRIGHTNESS_TONE_REFINEMENT[0];
  const last = BRIGHTNESS_TONE_REFINEMENT[BRIGHTNESS_TONE_REFINEMENT.length - 1];
  if (multiplier <= first[0]) return first.slice(1);
  if (multiplier >= last[0]) return last.slice(1);

  for (let i = 0; i < BRIGHTNESS_TONE_REFINEMENT.length - 1; i++) {
    const a = BRIGHTNESS_TONE_REFINEMENT[i];
    const b = BRIGHTNESS_TONE_REFINEMENT[i + 1];
    if (multiplier < a[0] || multiplier > b[0]) continue;
    const t = (multiplier - a[0]) / (b[0] - a[0]);
    return a.slice(1).map((value, index) =>
      value + ((b[index + 1] - value) * t));
  }
  return first.slice(1);
}

/**
 * The complete editable native tone treatment for CSS brightness.
 *
 * Darkening is unchanged because Highlights/Shadows were calibrated only for
 * the positive Exposure rolloff. Brightening interpolates through the two
 * measured native fits and clamps every native field to its accepted range.
 *
 * `colorRisk` addresses bright saturated content by continuously removing the
 * extra tone refinement. At 1 it produces Exposure-only. `darkColorRisk`
 * addresses deep saturated content without discarding the useful Highlights
 * control: at the measured 1.18 anchor it changes
 *   E 0.10 / H 0.55 / S -0.15
 * to
 *   E 0.07 / H 0.55 / S -0.30.
 * That visible change reins in an over-bright result while recovering spread.
 * Both corrections scale in from brightness 1 to 1.18 and remain bounded.
 */
export function toneAdjustmentsForBrightness(
  multiplier,
  { colorRisk = 0, darkColorRisk = 0 } = {},
) {
  const [exposureCorrection, highlights, shadows] =
    brightnessToneRefinement(multiplier);
  const refinementWeight = 1 - Math.max(0, Math.min(1, colorRisk));
  const darkRisk = Math.max(0, Math.min(1, darkColorRisk));
  const brighteningWeight = Math.max(
    0,
    Math.min(1, (multiplier - 1) / 0.18),
  );
  const exposure = Math.max(
    -1,
    Math.min(
      1,
      exposureForBrightness(multiplier)
        + (exposureCorrection * refinementWeight)
        - (0.03 * darkRisk * brighteningWeight),
    ),
  );
  const out = { exposure: +exposure.toFixed(4) };
  if (highlights !== 0 && refinementWeight !== 0) {
    out.highlights = +(highlights * refinementWeight).toFixed(4);
  }
  const adjustedShadows = Math.max(
    -1,
    Math.min(
      1,
      (shadows * refinementWeight)
        - (0.15 * darkRisk * brighteningWeight),
    ),
  );
  if (adjustedShadows !== 0) {
    out.shadows = +adjustedShadows.toFixed(4);
  }
  return out;
}

/**
 * Express a CSS image-filter chain through Figma's native image adjustments.
 *
 * The result remains attached to the original IMAGE paint, so Figma exposes
 * its ordinary editable Color Adjust panel after import. A Figma-authored
 * `.fig` confirms these values are stored directly in `paintFilter`; no shader,
 * library, plugin or second raster is involved.
 *
 * CSS and Figma do not use identical transfer functions or operation ordering.
 * This is therefore a calibrated editable translation, not pixel identity.
 * Returns null only when the chain contains a primitive Figma cannot express.
 */
function paintFilterFromCss(filter, sourceColorProfile = null) {
  ...
  for (const { fn, amount } of ops) {
    ...
    if (fn === 'grayscale') {
      saturationScale *= 1 - Math.max(0, Math.min(1, amount));
      hasSaturation = true;
    } else if (fn === 'saturate') {
      saturationScale *= Math.max(0, amount);
      hasSaturation = true;
    } else if (fn === 'contrast') {
      contrastScale *= Math.max(0, amount);
      hasContrast = true;
    } else if (fn === 'brightness') {
      brightnessScale *= Math.max(0, amount);
      hasBrightness = true;
    } else {
      // Invert and sepia have no native Color Adjust field.
      return null;
    }
  }

  const out = {};
  // The UI calls this control Saturation; the kiwi field is named `vibrance`.
  const vibrance = Math.max(-1, Math.min(1, saturationScale - 1));
  const contrast = contrastForCss(contrastScale);
  // Source risk only attenuates the extra tone controls while authored colour
  // remains. Full grayscale keeps the calibrated grayscale treatment; partial
  // grayscale proportionally reduces the source-colour influence.
  const retainedColor = Math.max(0, Math.min(1, saturationScale));
  const colorRisk = brightnessScale > 1
    ? sourceColorRisk(sourceColorProfile) * retainedColor
    : 0;
  const darkColorRisk = brightnessScale > 1
    ? sourceDarkColorRisk(sourceColorProfile) * retainedColor
    : 0;
  const tone = toneAdjustmentsForBrightness(brightnessScale, {
    colorRisk,
    darkColorRisk,
  });
  if (hasSaturation && vibrance !== 0) out.vibrance = vibrance;
  if (hasContrast && contrast !== 0) out.contrast = contrast;
  if (hasBrightness) {
    if (tone.exposure !== 0) out.exposure = tone.exposure;
    if (tone.highlights !== undefined) out.highlights = tone.highlights;
    if (tone.shadows !== undefined) out.shadows = tone.shadows;
  }
  return out;
}

/**
 * Return whether a native filter chain can benefit from source-colour
 * analysis, and how much authored colour remains after its saturation ops.
 */
function sourceColorRetentionForFilter(filter) {
  if (!Array.isArray(filter?.ops) || !filter.ops.length) return 0;
  let brightnessScale = 1;
  let saturationScale = 1;
  for (const { fn, amount } of filter.ops) {
    if (typeof amount !== 'number' || !Number.isFinite(amount)) return 0;
    if (fn === 'brightness') brightnessScale *= Math.max(0, amount);
    else if (fn === 'grayscale') {
      saturationScale *= 1 - Math.max(0, Math.min(1, amount));
    } else if (fn === 'saturate') saturationScale *= Math.max(0, amount);
    else if (fn !== 'contrast') return 0;
  }
  if (brightnessScale <= 1 || saturationScale <= 0) return 0;
  return Math.max(0, Math.min(1, saturationScale));
}
...
async function handleImage(slide, el, ctx) {
  // Prefer Figma's native adjustments: one original image paint, immediately
  // editable through the standard Color Adjust panel, with no external asset.
  let source = ctx.resolveMedia(el.src);
  let sourceColorProfile = null;
  if (sourceColorRetentionForFilter(el.filter) > 0
      && typeof ctx.imageOps?.analyzeSourceColor === 'function') {
    try {
      sourceColorProfile = await ctx.imageOps.analyzeSourceColor(source);
    } catch (error) {
      // Analysis is a fidelity refinement, not a reason to lose an otherwise
      // valid image. Preserve the previous global calibration on failure.
      ctx.warn?.(
        `source color analysis failed; using global editable filter calibration`
        + `${error?.message ? ` (${error.message})` : ''}`,
      );
    }
  }
  const paintFilter = paintFilterFromCss(el.filter, sourceColorProfile);
  if (el.filter && paintFilter === null && (el.filter.invert || el.filter.forceWhite)) {
    // These two legacy mask operations have no native Color Adjust equivalent.
    source = await bakeImageFilter(source, el.filter, el.width, ctx.imageOps);
  } else if (el.filter && paintFilter === null && (el.filter.css || el.filter.ops?.length)) {
    ctx.warn?.(`image filter "${el.filter.css ?? el.filter.raw ?? ''}" has no editable Figma equivalent`);
  }
  ...
  if (node?.fillPaints?.length) {
    for (const p of node.fillPaints) {
      ...
      const cropTransform = objectPositionCropTransform(el, p);
      if (cropTransform) {
        // The plugin API calls this mode CROP. In the Slides kiwi schema the
        // same affine-image mode is named STRETCH.
        p.imageScaleMode = 'STRETCH';
        p.transform = cropTransform;
      }
      if (paintFilter && Object.keys(paintFilter).length) p.paintFilter = { ...paintFilter };
...
function offsetElement(el, dx, dy) {
  ...
  if (Array.isArray(shifted.children)) {
    shifted.children = shifted.children.map((child) => offsetElement(child, dx, dy));
  }
  return shifted;
}

async function handleMaskedGroup(target, el, ctx) {
  const frame = target.addFrame(el.x, el.y, el.width, el.height, {
    direction: 'NONE',
    spacing: 0,
    name: 'CSS Mask',
  ...
  if (frame?._node) {
    frame._node.frameMaskDisabled = true;
    frame._node.fillPaints = [];
    frame._node.strokePaints = [];
  }

  const maskPaint = buildCssLinearPaint(el.mask);
  if (!maskPaint) {
    ctx.warn?.('linear mask-image could not be converted, so its content was emitted without the mask');
    for (const child of el.children ?? []) {
      await applyElement(frame, offsetElement(child, -el.x, -el.y), ctx);
    }
    return;
  }

  const maskNode = frame.addRectangle(0, 0, el.width, el.height, {
    fill: '#FFFFFF',
    name: 'CSS linear mask',
  });
  maskNode.fillPaints = [maskPaint];
  maskNode.mask = true;
...
// matter: a missing id and a non-rectangular clip need different diagnostics,
// while a clip we simply failed to find would otherwise be silently
...
// can express, `{ unsupported: id }` when one is present but not expressible,
// or `{ missing: id }` when the reference names no definition. Nested clips
...
function resolveClipAt(clipSpans, clipDefs, groupSpans, pos) {
  ...
  for (const s of enclosing) {
    if (!clipDefs.has(s.ref)) return { missing: s.ref };
...
function rasterSize(width, height) {
  ...
  // TILE has one scale for both axes. Keep that scale tied to the common
  // sampling density rather than deriving it from either rounded dimension:
  // width and height can round in opposite directions, and only one axis may
  // be the one that hit the cap.
  return {
    width: Math.max(1, Math.round(w * shrink)),
    height: Math.max(1, Math.round(h * shrink)),
    tileScale: 1 / (PATTERN_TILE_SUPERSAMPLE * shrink),
  };
}
...
export function parseSvgShapes(innerMarkup, opts = {}) {
  ...
  while ((m = tagRe.exec(expanded)) !== null) {
    ...
    if (shapes.length > shapeStart) {
      ...
      // gone. Missing, unsupported, and rectangular clips stay separate so
      // the emitter can explain the exact loss rather than guessing.
      if (clipSpans.length) {
        const clip = resolveClipAt(clipSpans, clipDefs, groupSpans, m.index);
        if (clip?.missing) emitted.clipMissing = clip.missing;
        else if (clip?.unsupported) emitted.clipUnsupported = clip.unsupported;
...
async function handleSvg(slide, el, ctx) {
  ...
  const applyPattern = async (node, id, sh) => {
    ...
    let png;
    try {
      png = await ctx.imageOps.rasterizeSvg(doc, {
        width: raster.width,
    ...
    const paint = await slide.createImagePaint({ bytes: png, mime: 'image/png' }, {
      ...
      scale: period ? raster.tileScale : 0.5,
    });
  ...
  for (const sh of shapes) {
    ...
    if (sh.clipMissing) {
      if (!reportedClips.has(sh.clipMissing)) {
        reportedClips.add(sh.clipMissing);
        ctx.warn?.(
          `SVG clip path #${sh.clipMissing} does not exist, so its content was dropped `
          + 'rather than drawn without the requested clip',
        );
      }
      continue;
    }
