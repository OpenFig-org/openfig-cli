...
${rows.map(({ n, scoreStr, renderUri, renderDims, refUri, refDims, overlayUri, offCount, offPct, meanDelta, offDelta }) => {
  ...
  const ssimOk = hasRef && ssim >= 0.90;
  const offDeltaOk = !hasRef || offDelta <= 20;
  // Combined verdict
  const allPass = !hasRef || (ssimOk && offDeltaOk);
  const badgeHtml = !hasRef ? '' : `<span class="badge ${allPass ? 'pass' : 'fail'}">${allPass ? 'PASS' : 'FAIL'}</span>`;
  // Metrics table (each value colored by its own threshold)
  const metricsHtml = !hasRef ? '' : `<table class="metrics">
    <tr><td class="label">SSIM</td><td class="${ssimOk ? 'g' : 'r'}">${scoreStr}</td></tr>
    <tr><td class="label">Pixels off</td><td>${offCount.toLocaleString()} (${offPct}%)</td></tr>
    <tr><td class="label">Mean \u0394</td><td>${meanDelta}</td></tr>
    <tr><td class="label">Severity</td><td class="${offDeltaOk ? 'g' : 'r'}">${offDelta}/255</td></tr>
  </table>`;
  return `
