...
export const canvasImageOps = {
  ...
  async analyzeSourceColor(src) {
    const d = await decode(src);
    try {
      const size = sourceColorAnalysisSize(d.width, d.height);
      const canvas = surface(size.width, size.height);
      const ctx = canvas.getContext('2d', { willReadFrequently: true });
      // Nearest-neighbour sampling is intentional. It avoids baking a
      // host-specific resampling kernel into a coarse statistical signal.
      ctx.imageSmoothingEnabled = false;
      ctx.drawImage(d.img, 0, 0, size.width, size.height);
      const pixels = ctx.getImageData(0, 0, size.width, size.height);
      return sourceColorProfileFromRgba(pixels.data, size);
    } finally {
      d.release();
    }
  },
