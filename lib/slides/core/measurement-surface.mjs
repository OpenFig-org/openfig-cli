...
async function clampWeightsToAvailableFaces(surface, host) {
  ...
  try {
    clamped = await surface.evaluate(({ realm }) => {
      ...
      // family -> the weights it provides. A variable font reports a range
      // ("300 700"); everything between the ends is available from it.
      const faces = new Map();
      for (const face of document.fonts) {
        ...
        if (!faces.has(family)) faces.set(family, { exact: new Set(), ranges: [] });
  ...
  if (clamped?.length && typeof host.log === 'function') {
    for (const { family, want, got, count } of clamped) {
      host.log(
