...
export function validateDeck(message, opts = {}) {
  ...
  // Variables and variable sets hang off a container Figma provides rather than
  // one the file carries, so their parent guid legitimately names no node here.
  // Checking them reported 50 "dangling" parents on a file whose canvas tree
  // was intact.
  const IMPLICIT_PARENT_TYPES = new Set(['VARIABLE', 'VARIABLE_SET']);
  const dangling = new Set();
  for (const c of nodes) {
    if (IMPLICIT_PARENT_TYPES.has(c.type)) continue;
