 ...
 *
 * PNG is big-endian throughout, so every setUint32 here passes `false`. The
 * .fig chunk framing next door is little-endian — do not copy one to the other.
 *
 * @returns {Uint8Array} PNG bytes
 */
export function createPlaceholderThumbnail(width = 400, height = 260) {
  const row = new Uint8Array(1 + width * 3).fill(0xff); row[0] = 0;
  const raw = new Uint8Array(height * row.length);
  for (let i = 0; i < height; i++) raw.set(row, i * row.length);
  // pako's zlib-wrapped deflate, not fflate's `deflateSync` — that one emits
  // *raw* DEFLATE, which PNG's IDAT rejects. pako is already the codec's
  // deflate on the .fig path and runs unchanged in a browser.
  ...
  const chunk = (type, data) => {
    // Length + 4-byte ASCII type + data + CRC over type||data, in one buffer.
    const out = new Uint8Array(12 + data.byteLength);
    const dv = new DataView(out.buffer);
    dv.setUint32(0, data.byteLength, false);
    out.set(enc.encode(type), 4);
    out.set(data, 8);
    dv.setUint32(8 + data.byteLength, crc32(out.subarray(4, 8 + data.byteLength)), false);
    return out;
  };
  const concat = (parts) => {
