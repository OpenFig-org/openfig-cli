...
export class BrowserConversionHost {
  ...
  constructor({
    ...
    // Stage timing to the console. Off by default: a conversion that stops
    // making progress otherwise gives nothing away, and this is the only way
    // to see which stage it stopped in without attaching a debugger.
    if (debug) {
      this.debug = (line) => console.log('[openfig] ' + line);
    }
