...
import { applyCssFilterRgba } from './css-filter-pixels.mjs';
import {
...
export const sharpImageOps = {
  ...
  async analyzeSourceColor(src) {
    const from = input(src);
    const meta = await sharp(from).metadata();
    const size = sourceColorAnalysisSize(meta.width ?? 0, meta.height ?? 0);
    const { data } = await sharp(from)
    ...
    return sourceColorProfileFromRgba(data, size);
  },
  ...
  async bakeFilter(src, filter, opts = {}) {
    ...
    if (filter.ops) {
      // Decode once, apply the same ordered byte arithmetic as the browser
      // host, then encode losslessly. This covers partial grayscale, sepia and
      // saturation rather than silently dropping or over-applying them.
      const { data, info } = await pipeline
        .ensureAlpha()
        .raw()
        .toBuffer({ resolveWithObject: true });
      applyCssFilterRgba(data, filter.ops);
      return sharp(data, {
        raw: {
          width: info.width,
          height: info.height,
