...
- **Conformance tests for the vector network format.** `test/slides/vector-network-layout.test.mjs` holds the deck emitter to the layout and `test/rasterizer/decode-vnb.test.mjs` pins curve classification, so neither can regress silently.
