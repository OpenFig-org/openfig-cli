...
import { FIGMA_AVAILABLE_FONTS, NAMEABLE_WEIGHTS, normalizeFont } from '../font-normalize.mjs';
...
// Parse the CSS `filter` values the image handoff recognises.
//
// Returns one of:
//   { forceWhite: true, raw }        `brightness(0) invert(1)` in that order —
//                                    every visible pixel becomes white. Kept as
//                                    a raster mask fallback because Figma has
//                                    no native editable equivalent.
//   { invert: 1, raw }               plain `invert(1)`, also a raster fallback.
//   { css, ops, raw }                colour transforms for native translation.
//   null                             nothing we can apply; the caller warns.
//
// `css` retains the authored string for diagnostics; `ops` decomposes the
// chain once for native-field translation and the raster helpers, without
// making either handoff host parse CSS again.
const RECOGNIZED_IMAGE_FILTERS = new Set([
  'grayscale',
  'brightness',
  'contrast',
  'invert',
...
function parseImageFilter(raw) {
  ...
  for (const tok of tokens) {
    ...
    if (!RECOGNIZED_IMAGE_FILTERS.has(fn)) return null; // blur, hue-rotate, drop-shadow…
...
function normalizeElement(el) {
  ...
  // Containers are not limited to Auto Layout. CSS masks also preserve a
  // subtree so their descendants stay inside the native mask boundary; every
  // such subtree still needs the same image-filter/text normalization as the
  // slide root.
...
export async function convertStandaloneCore(sourceHtml, host, opts = {}) {
  ...
  stage('openSurface');
  try {
    await prepareForMeasurement(surface, host, { readySelector: 'section', fitSelector: 'section', nameableWeights: NAMEABLE_WEIGHTS });
  ...
  for (const slide of manifestOut.slides) {
    for (const el of slide.elements ?? []) {
      if ((el.type === 'image' || el.type === 'svg') && el.unhandledFilter) {
        ...
        collector.warn(
          slide.index - 1,
          `${what} filter not applied: ${el.unhandledFilter} (native editable image controls: grayscale, brightness, contrast, saturate)`,
