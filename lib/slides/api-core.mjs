/**
 * openfig programmatic API — the portable half.
 *
 * High-level Deck / Slide / Symbol classes wrapping FigDeckCore.
 * Analogous to python-pptx's Presentation / Slide model.
 *
 * Nothing in this file touches a filesystem, so a browser can build a `.deck`
 * with it. `./api.mjs` subclasses `Deck`, keeps the module path and the
 * exported names the CLI and the `openfig-cli` package export already use, and
 * adds the three things only Node can do: open a file, save a file, and turn
 * an image or SVG *path* into bytes.
 *
 * Those three arrive as a `DeckIo` capability rather than as imports —
 * the same shape as `ConversionHost` in `core/host-contract.mjs`, and for the
 * same reason: a module-scope `import 'fs'` lands in the browser bundle
 * whether or not the branch that needs it ever runs. `DeckIo` is threaded
 * Deck → Slide → Shape → FrameProxy, so every object that can be handed a
 * path knows whether paths are supported here.
 *
 * Phases implemented:
...
import { FigDeckCore } from '../core/fig-deck-core.mjs';
...
/**
 * The environment-specific half of the deck writer.
 *
 * @typedef {object} DeckIo
 * @property {import('../core/image-ops.mjs').ImageOps|null} imageOps
 *   Sizing and thumbnailing. `sharpImageOps` in Node — the implementation that
 *   produced every recorded `.deck` baseline — `canvasImageOps` in a browser.
 * @property {((path: string) => Uint8Array)|null} readFileBytes
 *   Node only. Turns a path argument to `addImage`/`setImage`/`setImageFill`
 *   into bytes.
 * @property {((path: string) => string)|null} readFileText
 *   Node only. Same, for `addSVG`'s file form.
 */

/** No filesystem, no raster library: what a browser starts from. */
export const PORTABLE_DECK_IO = Object.freeze({
  imageOps: null,
  readFileBytes: null,
  readFileText: null,
});

function requireImageOps(io) {
  if (!io?.imageOps) {
    throw new Error(
      'This deck has no image implementation. Pass `imageOps` (sharpImageOps in Node, ' +
      'canvasImageOps in a browser) via opts, or use the Deck from lib/slides/api.mjs.',
    );
  }
  return io.imageOps;
}

/** Resolve an image argument to bytes, reading a path only where that is possible. */
function imageBytes(io, pathOrBuf) {
  if (typeof pathOrBuf !== 'string') {
    return pathOrBuf && typeof pathOrBuf === 'object' && !ArrayBuffer.isView(pathOrBuf)
      ? pathOrBuf.bytes
      : pathOrBuf;
  }
  if (!io?.readFileBytes) {
    throw new Error(
      `Image file paths are only supported by the Node deck writer: ${pathOrBuf}. ` +
      'Pass bytes, or a { bytes, mime } media record.',
    );
  }
  return io.readFileBytes(pathOrBuf);
}
...
export class Deck {
  /**
   * @param {FigDeckCore} figDeck
   * @param {string|null} [sourcePath]
   * @param {DeckIo} [io]
   */
  constructor(figDeck, sourcePath = null, io = PORTABLE_DECK_IO) {
    this._fd = figDeck;
    this._sourcePath = sourcePath;
    this._io = io;
  }

  /** The FigDeck class this Deck builds. `./api.mjs` swaps in the Node one. */
  static get FigDeckClass() { return FigDeckCore; }

  /** The `DeckIo` a Deck of this class gets when the caller names none. */
  static get defaultIo() { return PORTABLE_DECK_IO; }
   ...
   * @param {DeckIo} [opts.deckIo] - Overrides the class default.
   * @returns {Promise<Deck>}
   */
  static async create(opts = {}) {
    const fd = this.FigDeckClass.createEmpty({ name: opts.name ?? 'Untitled' });
    // `new this(...)`, so `Deck.create()` on the Node subclass returns a deck
    // that still has `save()`. `undefined` lets each class's own default apply.
    const deck = new this(fd, null, opts.deckIo);
  ...
  /**
   * Load a deck from `.deck` archive bytes. The byte counterpart of
   * `./api.mjs`'s `Deck.open(path)`.
   *
   * @param {Uint8Array} bytes
   * @param {object} [opts]
   * @param {DeckIo} [opts.deckIo]
   * @returns {Deck}
   */
  static fromBytes(bytes, opts = {}) {
    return new this(this.FigDeckClass.fromDeckBytes(bytes), null, opts.deckIo);
  }
  ...
  get slides() {
    return this._fd.getActiveSlides().map(n => new Slide(this._fd, n, this._io));
  }
  ...
  /**
   * Encode the deck as `.deck` archive bytes.
   *
   * The byte half of `save()`, which the Node subclass keeps. Identical work:
   * `saveDeck` is `validate()` + `toDeckBytes()` + a write, and its images
   * overlay reads an `imagesDir` that a deck built in memory never has.
   *
   * @returns {Promise<Uint8Array>}
   */
  async toBytes() {
    this._fd.validate();
    return this._fd.toDeckBytes();
  }
  ...
  addBlankSlide(opts = {}) {
    ...
    const slide = new Slide(fd, newSlide, this._io);
  ...
  addSlide(symbol, opts = {}) {
    ...
    return new Slide(fd, newSlide, this._io);
  }
...
export class Slide {
  constructor(figDeck, slideNode, io = PORTABLE_DECK_IO) {
    this._fd = figDeck;
    this._io = io;
  ...
  get shapes() {
    return this._fd.getChildren(nid(this._node))
      ...
      .map(n => new Shape(n, this._fd, this._io));
  }
   ...
   * @param {string|Uint8Array} pathOrBuf - A path only under the Node writer.
   * @param {object} [opts]
   * @param {import('../core/image-ops.mjs').ImageOps} [opts.imageOps]
   */
  async setImage(nameOrKey, pathOrBuf, opts = {}) {
    const key = this._resolveImageKey(nameOrKey);
    if (!key) throw new Error(`Image node not found: ${nameOrKey}`);

    const imageOps = opts.imageOps ?? requireImageOps(this._io);
    const imgBuf = imageBytes(this._io, pathOrBuf);

    const imgHash = await sha1Hex(imgBuf);

    const { width, height } = await imageOps.imageSize(imgBuf);

    // Bytes, not a temp file read back: `sharp`'s `toFile` and `toBuffer` were
    // verified byte-identical in task 2.4, and Canvas has no `toFile` at all.
    const thumbBytes = await imageOps.thumbnailPng(imgBuf);
    const thumbHash = await sha1Hex(thumbBytes);

    putImage(this._fd, imgHash, imgBuf);
    putImage(this._fd, thumbHash, thumbBytes);
   ...
   * @param {number} width
   * @param {number} height
   * @param {string|Buffer|{bytes: Uint8Array, mime?: string}} pathOrBuf
   *   An image file path, raw bytes, or a media record. The record form is
   *   what an in-memory handoff bundle hands out
   *   (`browser/memory-bundle.mjs`); it carries the mime type, which is the
   *   only reliable SVG signal once there is no filename.
   * @param {object} [opts]
   * @param {string} [opts.name]        - Node name
   * @param {string} [opts.scaleMode]   - 'FILL' | 'FIT' | 'CROP' | 'TILE' (default: 'FILL')
   * @param {number} [opts.cornerRadius]- Corner radius (default: 0)
   * @param {import('../core/image-ops.mjs').ImageOps} [opts.imageOps]
   *   Overrides the implementation used to size and thumbnail the image.
   *   Defaults to the deck's `DeckIo.imageOps` — `sharpImageOps` under
   *   `./api.mjs`, which produced every recorded baseline; a browser passes
   *   `canvasImageOps`.
  ...
  async addImage(pathOrBuf, opts = {}) {
    ...
    const record = pathOrBuf && typeof pathOrBuf === 'object' && !ArrayBuffer.isView(pathOrBuf)
      ? pathOrBuf
      : null;
    const imgBuf = imageBytes(this._io, pathOrBuf);
    // Sizing and thumbnailing need the mime to tell an SVG from a raster; the
    // hashed bytes never do, and are passed through untouched either way.
    // A path argument has already become bytes by here, exactly as before —
    // `sharp` was handed `imgBuf`, never the path.
    const sizeSource = record ?? imgBuf;

    const imgHash = await sha1Hex(imgBuf);
    const { width: origW, height: origH } = await imageOps.imageSize(sizeSource);

    // Generate thumbnail. Bytes, not a file read back: `sharp`'s `toFile` and
    // `toBuffer` were verified byte-identical in task 2.4, and a Canvas
    // implementation has no `toFile` at all.
    const thumbBytes = await imageOps.thumbnailPng(sizeSource);
    const thumbHash = await sha1Hex(thumbBytes);

    putImage(fd, imgHash, imgBuf);
    putImage(fd, thumbHash, thumbBytes);
  ...
  addText(textOrRuns, opts = {}) {
    ...
    if (styleDef && !isDetached) {
      ...
      const psName = opts.postscript ?? synthesizePostScript(family, style);
    ...
    if (isRuns && textOrRuns.some(runHasFormatting)) {
    ...
    const node = {
      ...
      textData,
      fontName,
      // The style name can only say Regular or Bold, so an authored 500 or 600
      // travels as a `wght` variation instead — Figma applies one continuously
      // along a family's weight axis, and ignores it on a family with none.
      // Written only when there is one, so text at 400 or 700 encodes exactly
      // as it always did.
      ...(Array.isArray(opts.fontVariations) && opts.fontVariations.length
        ? { fontVariations: deepClone(opts.fontVariations) }
        : {}),
      fontSize,
      lineHeight,
      letterSpacing,
  ...
  addSVG(x, y, width, svgPathOrBuf, opts = {}) {
    ...
    // Read SVG. Markup is handled everywhere; a path only under the Node
    // writer, which supplies `readFileText`.
    let svgStr;
    if (svgPathOrBuf.includes('<svg')) {
      svgStr = svgPathOrBuf;
    } else if (this._io?.readFileText) {
      svgStr = this._io.readFileText(svgPathOrBuf);
    } else {
      throw new Error(
        `SVG file paths are only supported by the Node deck writer: ${svgPathOrBuf}. ` +
        'Pass the SVG markup instead.',
      );
    }
  ...
  addFrame(x, y, width, height, opts = {}) {
    ...
    return new FrameProxy(fd, node, this._node, this._io);
  }

  /**
   * Register an image with the deck and return an IMAGE paint for it, without
   * creating a node.
   *
   * `addImage` makes a rectangle; a paint is what you want when the geometry
   * already exists and only its fill is an image — an SVG `pattern` fill on a
   * pie wedge is a path node whose `fillPaints` becomes a tiled image, exactly
   * as a gradient fill becomes a gradient paint. Emitting a rectangle there
   * would replace the wedge with a box.
   *
   * @param {string|Uint8Array|{bytes: Uint8Array, mime?: string}} pathOrBuf
   * @param {object} [opts]
   * @param {string} [opts.scaleMode] - 'FILL' | 'FIT' | 'CROP' | 'TILE'
   * @param {number} [opts.scale]
   *   Tile scale for `TILE`: the image's natural pixel size times this is the
   *   tile's size on the canvas. Ignored by the other modes.
   * @param {import('../core/image-ops.mjs').ImageOps} [opts.imageOps]
   * @returns {Promise<object>} a Paint
   */
  async createImagePaint(pathOrBuf, opts = {}) {
    const imageOps = opts.imageOps ?? requireImageOps(this._io);
    const record = pathOrBuf && typeof pathOrBuf === 'object' && !ArrayBuffer.isView(pathOrBuf)
      ? pathOrBuf
      : null;
  ...
  /**
   * Bytes for an image argument, resolving a path only where that is possible.
   *
   * Exposed for callers that must *inline* an asset rather than hand it to
   * `addImage` — rasterising an SVG `pattern` whose tile contains an `<image>`
   * means embedding that image in the markup as a `data:` URI, because neither
   * raster backend will resolve a reference out of the document.
   *
   * @param {string|Uint8Array|{bytes: Uint8Array}} pathOrBuf
   * @returns {Uint8Array}
   */
  imageSourceBytes(pathOrBuf) {
    return imageBytes(this._io, pathOrBuf);
  }
...
export class Shape {
  constructor(node, figDeck = null, io = PORTABLE_DECK_IO) {
    this._io = io;
    this._node = node;
    this._fd = figDeck;
  }
   ...
   *
   * @param {string|Uint8Array} pathOrBuf - Image bytes; a path only under the
   *   Node writer.
   * @param {object} [opts]
   * @param {string} [opts.scaleMode] - 'FILL' | 'FIT' | 'CROP' | 'TILE' (default: 'FILL')
   * @param {import('../core/image-ops.mjs').ImageOps} [opts.imageOps]
  ...
  async setImageFill(pathOrBuf, opts = {}) {
    ...
    const imgHash = await sha1Hex(imgBuf);
    const { width: origW, height: origH } = await imageOps.imageSize(imgBuf);
...
class FrameProxy {
  constructor(figDeck, frameNode, slideNode, io = PORTABLE_DECK_IO) {
    this._io = io;
    this._fd = figDeck;
   ...
   */
  addText(text, opts = {}) {
    // Auto-layout positions its children itself, so a coordinate handed in
    // here is noise and zeroing it is the honest thing to do. A frame with
    // `stackMode: 'NONE'` has no auto-layout: it positions children by their
    // own transform, exactly as a slide does. Those are the clip containers an
    // SVG `clipPath` produces, and zeroing coordinates inside one would stack
    // every clipped text node in the container's top-left corner.
    const positioned = this._node.stackMode === 'NONE';
    return Slide.prototype.addText.call(this, text, positioned ? opts : { ...opts, x: 0, y: 0 });
  }
  ...
  async createImagePaint(pathOrBuf, opts = {}) {
    return Slide.prototype.createImagePaint.call(this, pathOrBuf, opts);
  }

  imageSourceBytes(pathOrBuf) {
    return Slide.prototype.imageSourceBytes.call(this, pathOrBuf);
  }
 ...
 */
/**
 * Does this run need a style override of its own?
 *
 * `font` counts: a run set in a family of its own needs an override even with
 * no bold, italic or colour, or it falls back to styleID 0 and is written in
 * the paragraph's family. This predicate was written out twice — once to
 * decide whether to build the table at all, once per run inside it — and the
 * two drifted, so a family-only run passed the second check and never reached
 * it because of the first.
 */
/**
 * Figma's resolver matches on (family, postscript) together, and an empty
 * postscript makes it substitute a fallback even when the family is present.
 * `addText` has synthesised one for the node-level font since the beginning;
 * run overrides passed an empty string, so by this file's own model of the
 * resolver every styled run was a substitution candidate.
 */
function synthesizePostScript(family, style) {
  return family.replace(/\s+/g, '') + '-' + style.replace(/\s+/g, '');
}

function runHasFormatting(r) {
  return Boolean(
    r.bold || r.italic || r.underline || r.strikethrough || r.hyperlink || r.color || r.font
...
function buildRunOverrides(runs, baseFontName, styleIdForText, fd) {
  ...
  for (const run of runs) {
    const hasFormat = runHasFormatting(run);
    ...
    // The family belongs in the key too — without it, two runs alike in every
    // other respect but set in different families share one styleID, and the
    // second silently takes the first one's font.
    const fontKey = run.font ? 'f:' + run.font : '';
    // Same argument as the family: two runs alike but for their weight
    // variations are two different faces, and sharing a styleID would give the
    // second the first one's weight.
    const varKey = run.fontVariations?.length ? 'v:' + JSON.stringify(run.fontVariations) : '';
    const key = `${run.bold ? 'b' : ''}${run.italic ? 'i' : ''}${run.underline ? 'u' : ''}${run.strikethrough ? 's' : ''}${run.hyperlink ? 'h:' + run.hyperlink : ''}${colorKey}${fontKey}${varKey}`;
    ...
    if (!styleMap.has(key)) {
      ...
      // A run in a different family needs its own fontName even when it is
      // neither bold nor italic; inheriting the paragraph's family named a
      // face the run's text was never in, and one that need not exist there.
      const runFamily = run.font || baseFontName.family;
      if (run.bold || run.italic || runFamily !== baseFontName.family) {
        ...
        entry.fontName = {
          family: runFamily,
          style,
          postscript: synthesizePostScript(runFamily, style),
        };
        entry.fontVersion = '1';
      ...
      // `styleOverrideTable` entries are NodeChanges, the same message the text
      // node itself is — checked against the deck's own kiwi schema, not
      // assumed — so a run carries its authored weight exactly as the node
      // does. A run that sets no variation inherits the node's, which is why
      // the caller emits one on every run whose weight differs from the
      // paragraph's rather than only on the unnameable ones.
      if (run.fontVariations?.length) entry.fontVariations = deepClone(run.fontVariations);
...
// `Buffer.writeFloatLE` / `writeUInt32LE` are exactly `DataView.setFloat32(…,
// true)` / `setUint32(…, true)`, and `Buffer.alloc` is unpooled so `buf.buffer`
// was already zero-offset where these return a view over it. Byte-for-byte the
// same blob, without a Node global on the shared path.
function _encodeCommandsBlob(cmds, sx, sy) {
  ...
  const f32 = (v, o) => view.setFloat32(o, v, true);
  let off = 0;
  for (const c of cmds) {
    switch (c.type) {
      case 'M': buf[off++] = 1; f32(c.x * sx, off); off += 4; f32(c.y * sy, off); off += 4; break;
      case 'L': buf[off++] = 2; f32(c.x * sx, off); off += 4; f32(c.y * sy, off); off += 4; break;
      case 'C': buf[off++] = 4;
        f32(c.c1x * sx, off); off += 4; f32(c.c1y * sy, off); off += 4;
        f32(c.c2x * sx, off); off += 4; f32(c.c2y * sy, off); off += 4;
        f32(c.x * sx, off); off += 4; f32(c.y * sy, off); off += 4; break;
...
function _buildVectorNetworkBlob(allPathCmds, { emitRegions = true } = {}) {
    ...
    for (const c of pathCmds) {
      if (c.type === 'M') {
        ...
        if (prevVtx >= 0) {
          ...
          segments.push({ s: prevVtx, tsx: 0, tsy: 0, e: vi, tex: 0, tey: 0 });
        }
        ...
        if (prevVtx >= 0) {
          currentLoop.push(segments.length);
          segments.push({ s: prevVtx, tsx: c.c1x - prevX, tsy: c.c1y - prevY, e: vi, tex: c.c2x - c.x, tey: c.c2y - c.y });
        }
        prevVtx = vi; prevX = c.x; prevY = c.y;
      } else if (c.type === 'Z') {
        if (prevVtx >= 0 && prevVtx !== firstVtx) {
          currentLoop.push(segments.length);
          segments.push({ s: prevVtx, tsx: 0, tsy: 0, e: firstVtx, tex: 0, tey: 0 });
        }
  ...
  // Figma's verified layout, little-endian. Byte-exact on the reference corpus in
  // openfig-core/test-fixtures/vector-network/ (17 blobs, 6 files, format v101/v106):
  //
  ...
  //   region      : packed(u32) = styleID<<1|windingRule, numLoops(u32),
  //                 then per loop: segCount(u32) + segIndices(u32 * segCount)
  //
  // We author with handleMirroring = 0 and segment word0 = 0, both values Figma
  // itself writes. This emitter previously wrote a one-word rotation of the above —
  // a 16-byte header, [x, y, mirroring], [start, …, type], and a per-region trailing
  // word — plus the constant 4 in the mirroring and type slots, a value that appears
  // in none of the reference blobs and so identified our output on sight.
  let regSize = 0;
  for (const loops of regions) {
    regSize += 8; // packed + numLoops
    for (const loop of loops) regSize += 4 + loop.length * 4; // segCount + indices
  }
  const totalSize = 12 + vertices.length * 12 + segments.length * 28 + regSize;
  const buf = new Uint8Array(totalSize);
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  const f32 = (v, o) => view.setFloat32(o, v, true);
  const u32 = (v, o) => view.setUint32(o, v, true);
  let off = 0;

  // Header
  u32(vertices.length, off); off += 4;
  u32(segments.length, off); off += 4;
  u32(regions.length, off); off += 4;

  // Vertices: handleMirroring(u32) x(f32) y(f32)
  ...
  // Segments: word0(u32) start(u32) tsx(f32) tsy(f32) end(u32) tex(f32) tey(f32).
  // No segment-type field: curve-ness is carried by the tangents alone.
  ...
  // Regions: packed(u32) numLoops(u32) + [segCount(u32) + segIndices(u32*n)]*numLoops
  for (const loops of regions) {
    u32(1, off); off += 4; // styleID 0, windingRule NONZERO
    u32(loops.length, off); off += 4;
    for (const loop of loops) {
      u32(loop.length, off); off += 4;
      for (const si of loop) { u32(si, off); off += 4; }
    }
  }
 ...
 *
 * WebCrypto rather than `node:crypto`: `globalThis.crypto.subtle` is the one
 * spelling both environments already have (Node ≥ 18, and every browser in a
 * secure context), so hashing needs no adapter member at all. Same digest, and
 * `browser/browser-conversion-host.mjs` already computes the other side of the
 * image join this way.
 *
 * Async, unlike the `createHash` it replaces — every call site was already
 * inside an `async` method, so awaiting costs nothing.
 *
 * @param {Uint8Array} bytes
...
/**
 * Record an image against the deck, keyed by the sha1 hex that names its
 * archive entry.
 *
 * Replaces `copyToImagesDir`, which staged every image through a file in
 * `tmpdir()` purely so `copyFileSync` had a source — `FigDeck.images` is the
 * map `toDeckBytes` reads either way. Entry *contents* are unchanged; entry
 * *order* within `images/` becomes insertion order (slide by slide, source
 * before thumbnail) instead of whatever `readdir` returned, which was the
 * filesystem's choice and never a contract. See task 4b.4.
 *
 * First write wins, as `if (!existsSync(dest))` did: two nodes sharing an
 * image agree on its bytes by construction, since the key is their hash.
 */
function putImage(fd, hash, bytes) {
  if (!fd.images.has(hash)) fd.images.set(hash, bytes);
}
