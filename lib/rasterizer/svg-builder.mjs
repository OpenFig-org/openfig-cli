 ...
 *
 * Binary layout (all little-endian), verified byte-exact on Figma-authored blobs:
 *   Header:   vertexCount(u32), segmentCount(u32), regionCount(u32)  — 12 bytes
 *   Vertices: word0(u32), x(f32), y(f32)  — 12 bytes each
 *   Segments: word0(u32), startVertex(u32), tangentStartX(f32), tangentStartY(f32),
 *             endVertex(u32), tangentEndX(f32), tangentEndY(f32)  — 28 bytes each
 *   Regions:  packed(u32) = styleID<<1|windingRule, numLoops(u32),
 *             per loop: segCount(u32) + segIndices(u32[segCount])
 *
 * word0 is 0 on every reference vertex and segment; its meaning is unknown and it is
 * never used. There is no segment-type field: a segment is straight if and only if all
 * four tangent components are zero.
 */
export function decodeVnb(blobs, blobIdx, normalizedSize, nodeSize) {
  ...
  // Parse vertices: word0, x, y
  const verts = [];
  for (let i = 0; i < numVerts; i++) {
    off += 4; // word0 — unknown, unused
    const x = buf.readFloatLE(off) * scaleX; off += 4;
    const y = buf.readFloatLE(off) * scaleY; off += 4;
    verts.push({ x, y });
  }

  // Parse segments: word0, startVertex, tangentStart, endVertex, tangentEnd
  const segs = [];
  for (let i = 0; i < numSegs; i++) {
    ...
    segs.push({ sv, tsx, tsy, ev, tex, tey });
  }

  const segToPathCmd = (seg, start, end) => {
    if (seg.tsx === 0 && seg.tsy === 0 && seg.tex === 0 && seg.tey === 0) {
  ...
  for (let r = 0; r < numRegions; r++) {
    ...
    off += 4; // packed = styleID<<1|windingRule — not needed for geometry
