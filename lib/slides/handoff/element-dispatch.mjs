...
// Figma's exposure control, measured. CSS states brightness as a multiplier on
// luminance; Figma's `exposure` is its own curve and is not linear in it, so
// the mapping is a measured table rather than arithmetic. Each entry is
// (exposure, resulting luminance as a multiple of the unadjusted image),
// sampled from a compatibility-reference output of a sweep from -1 to 1.
const EXPOSURE_CURVE = [
  [-1, 0.155], [-0.5, 0.436], [-0.25, 0.684], [0, 1.0],
  [0.25, 1.351], [0.5, 1.697], [1, 2.259],
];

function exposureForBrightness(multiplier) {
  const c = EXPOSURE_CURVE;
  if (multiplier <= c[0][1]) return c[0][0];
  if (multiplier >= c[c.length - 1][1]) return c[c.length - 1][0];
  for (let i = 0; i < c.length - 1; i++) {
    const [e0, r0] = c[i];
    const [e1, r1] = c[i + 1];
    if (multiplier >= r0 && multiplier <= r1) {
      return e0 + ((multiplier - r0) / (r1 - r0)) * (e1 - e0);
    }
  }
  return 0;
}
...
function paintFilterFromCss(filter) {
  ...
  for (const { fn, amount } of ops) {
    ...
    else if (fn === 'contrast') out.contrast = Math.max(-1, Math.min(1, amount - 1));
...
//
// Deliberately narrower than SVG_DEFINITION_TAGS. `defs`, `mask` and `marker`
// have the same latent problem and the same fix, but their contents are
// painted today and are recorded by the byte-baseline fixtures; correcting
// them is its own change rather than a passenger on this one.
const SVG_CONSUMED_DEFINITION_TAGS = 'clipPath|pattern';
...
export function parseSvgShapes(innerMarkup, opts = {}) {
  ...
  // Only <image> consults this, so only pay for it when there is one. The
  // other shape branches have the same latent problem — a <rect> inside a
  // <mask> is scanned and painted today — but widening the rule here would
  // move geometry the byte-baseline fixtures record, so it is left as a
  // separate change rather than smuggled in with this one.
  ...
  const consumedSpans = /<(?:clipPath|pattern)\b/i.test(expanded)
