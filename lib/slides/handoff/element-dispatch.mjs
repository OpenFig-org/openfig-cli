...
// Bake a parsed CSS image filter into raster bytes. Returns the source
// unchanged when the filter contains no colour operation.
//
...
async function bakeImageFilter(src, filter, displayWidth, imageOps) {
  if (!filter || (!filter.invert && !filter.forceWhite && !filter.ops?.length)) return src;
  if (!imageOps) throw new Error('bakeImageFilter: imageOps is required (ctx.imageOps was not set)');

  const key = filter.forceWhite
    ? 'forceWhite'
    : filter.css ?? filter.raw ?? (filter.invert ? 'invert' : JSON.stringify(filter.ops));
...
function authoredImageFilterCss(filter) {
  if (filter?.css) return filter.css;
  if (filter?.raw) return filter.raw;
  if (filter?.forceWhite) return 'brightness(0) invert(1)';
  if (filter?.invert) return 'invert(1)';
  return (filter?.ops ?? []).map(({ fn, amount }) => `${fn}(${amount})`).join(' ');
}

async function handleImage(slide, el, ctx) {
  // Every supported colour filter is baked automatically. The rendered fill
  // is what a user sees immediately after import; the untouched source is kept
  // as an invisible second paint on the same node for lossless recovery.
  const originalSource = ctx.resolveMedia(el.src);
  const hasRasterFilter = Boolean(
    el.filter && (el.filter.invert || el.filter.forceWhite || el.filter.ops?.length),
  );
  const source = hasRasterFilter
    ? await bakeImageFilter(originalSource, el.filter, el.width, ctx.imageOps)
    : originalSource;
  ...
  if (hasRasterFilter && node?.fillPaints?.length) {
    const originalPaint = await slide.createImagePaint(originalSource, {
      scaleMode: opts.scaleMode,
      imageOps: ctx.imageOps,
    });
    originalPaint.visible = false;
    if (el.blendMode) originalPaint.blendMode = el.blendMode;
    node.fillPaints.push(originalPaint);

    const renderedPaint = node.fillPaints.find((paint) => paint.type === 'IMAGE' && paint.visible !== false);
    node.pluginData = [
      ...(node.pluginData ?? []),
      {
        pluginID: 'org.openfig',
        key: 'css-image-filter',
        value: JSON.stringify({
          version: 1,
          css: authoredImageFilterCss(el.filter),
          renderedImage: renderedPaint?.image?.name ?? '',
          sourceImage: originalPaint.image?.name ?? '',
        }),
      },
    ];
  }
