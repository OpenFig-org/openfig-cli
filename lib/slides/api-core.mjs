...
import { encodeCommandsBlob, encodeVectorNetworkBlob } from 'openfig-core';
...
export class Slide {
  ...
  addText(textOrRuns, opts = {}) {
    ...
    if (styleDef && !isDetached) {
      ...
      // what reference files emit for standard Google Fonts (Inter-Bold,
...
/**
 * Encode path commands into a `commandsBlob`.
 *
 * Delegates to `openfig-core`. This package previously carried a byte-for-byte
 * duplicate; verified identical across move/line, cubic, closed, scaled and
 * negative-coordinate cases before removal. A format described in two places is
 * a format that eventually disagrees with itself — the vectorNetworkBlob layout
 * did exactly that.
 */
function _encodeCommandsBlob(cmds, sx, sy) {
  return encodeCommandsBlob(cmds, sx, sy);
}
