...
export async function convertHandoffBundle(bundlePath, outDeckPath, opts = {}) {
  ...
  const { deck, bytes, noWrapDiagnostics, warnings } = await convertHandoffBundleToBytes(bundle, {
    ...opts,
    deckClass: Deck,
  ...
  // The extraction stage prints its own warnings before this runs, so these go
  // out here rather than being folded into that transcript — they belong to a
  // later stage and would otherwise be returned to a caller and never seen.
  if (warnings.length && !opts.silent) {
    const log = opts.warnLogger ?? ((s) => console.warn(s));
    log(`\nconvert-html: ${warnings.length} warning type(s) from deck emission:`);
    for (const w of warnings) {
      const times = w.count > 1 ? ` x${w.count}` : '';
      log(`  [slide ${w.slideIdx + 1}]${times} ${w.msg}`);
    }
  }

  writeFileSync(resolve(outDeckPath), bytes);
  return { deck, scratchDir: scratch, bundle, warnings };
}
