...
function renderRect(deck, node) {
  ...
  const fillSvg = `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="${rx}" ry="${rx}" fill="${fill}"/>`;
...
function renderFrame(deck, node) {
  ...
  if (imgFill) {
    ...
    const fill = resolveFill(getFillPaints(node)) ?? 'none';
    if (fill !== 'none') bg = `<rect x="0" y="0" width="${w}" height="${h}" rx="${rx}" ry="${rx}" fill="${fill}"/>`;
  }
  const strokeSvg = rectStrokeSvg(0, 0, w, h, rx, stroke);

  const parts = [defs, bg, strokeSvg, inner].filter(Boolean).join('\n');
  if (!parts) return '';
