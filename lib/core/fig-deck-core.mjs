...
export class FigDeckCore {
  ...
  validate() {
    ...
    // Resolve the first visible paint to a background descriptor:
    //   { solid: true, color }  — solid fill, safe to compare against
    //   { solid: false }        — image/gradient, cannot compute a ratio
    //   null                    — no fill, inherit from parent
    function resolveFill(paints) {
      if (!paints || paints.length === 0) return null;
      const paint = paints.find(p => p.visible !== false && p.opacity !== 0);
      if (!paint) return null;
      if (paint.type === 'SOLID' && paint.color) return { solid: true, color: paint.color };
      return { solid: false };
    }

    function extractTextColor(paints) {
      if (!paints || paints.length === 0) return null;
    ...
    for (const slide of this.getActiveSlides()) {
      ...
      const slideFill = resolveFill(slide.fillPaints);
      const slideBg = slideFill?.solid ? slideFill.color : { r: 1, g: 1, b: 1 };
      const slideBgKnown = slideFill ? slideFill.solid : true;

      // Walk descendants carrying the nearest solid background. Follow
      // INSTANCE → SYMBOL links so we reach TEXT inside components.
      const walk = (rootId, bgColor, bgKnown, visited) => {
        const node = this.getNode(rootId);
        if (!node) return;

        let effBg = bgColor;
        let effKnown = bgKnown;
        // TEXT fillPaints is the text colour, not a background — skip it here.
        if (rootId !== slideId && node.type !== 'TEXT') {
          const fill = resolveFill(node.fillPaints);
          if (fill) {
            effKnown = fill.solid;
            if (fill.solid) effBg = fill.color;
          }
        }

        if (node.type === 'TEXT' && effKnown) {
          const textColor = extractTextColor(node.fillPaints);
          if (textColor) {
            const bgLum = luminance(effBg.r, effBg.g, effBg.b);
            const textLum = luminance(textColor.r, textColor.g, textColor.b);
            const ratio = contrastRatio(bgLum, textLum);
            if (ratio < 2) {
              const nodeId = nid(node);
              let msg = `TEXT ${nodeId} "${node.name || ''}": contrast ratio ${ratio.toFixed(2)}:1 against background is below 2:1`;
              if (node.colorVar) {
        ...
        if (node.type === 'INSTANCE' && node.symbolData?.symbolID) {
          ...
          if (!visited.has(symNid)) {
            visited.add(symNid);
            walk(symNid, effBg, effKnown, visited);
          }
        }

        for (const child of this.getChildren(rootId)) {
          walk(nid(child), effBg, effKnown, visited);
        }
      };
      walk(slideId, slideBg, slideBgKnown, new Set());
    }
