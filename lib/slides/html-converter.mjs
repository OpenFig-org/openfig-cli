...
// resolved to blob: URLs can be matched back to a file on disk. Hashing is the
// only reliable join: the runtime hands out opaque blob URLs that carry no
// trace of the manifest uuid they came from.
function indexAssetsBySha1(mediaMap) {
  const bySha = new Map();
  for (const asset of Object.values(mediaMap)) {
    const sha = createHash('sha1').update(readFileSync(asset.path)).digest('hex');
    if (!bySha.has(sha)) bySha.set(sha, asset);
  }
  return bySha;
}

// Hash every loaded <img> in the page and pair it with its decoded asset.
// Returns a Map of the live src (blob: URL) → asset record.
async function mapImageSrcsToAssets(page, sha1ToAsset) {
  const hashed = await page.evaluate(async () => {
    const out = [];
    for (const img of document.querySelectorAll('img')) {
      ...
      try {
        ...
        // missing image rather than failing the whole build.
      }
    }
    return out;
  });
...
async function inlineSvgImages(page, srcToAsset) {
  const patches = [];
  for (const [src, asset] of srcToAsset) {
    if (asset.mime !== 'image/svg+xml') continue;
    const bytes = readFileSync(asset.path);
    patches.push({ src, dataUrl: `data:image/svg+xml;base64,${bytes.toString('base64')}` });
  }
...
// Repoint raster <img> srcs at the scratch media dir. Runs *after* extraction:
...
async function rewriteDomImageSrcs(page, srcToAsset) {
...
export async function convertStandaloneHtml(htmlPath, outDeckPath, opts = {}) {
  ...
  // the template and apply the saved tweak state. Replaying that mapping in
  // Node means re-implementing whatever classes and CSS vars a given deck's
  // controls script happens to use — which only ever holds for the deck it was
  // written against. The page is the authority on its own tweaks.
  const collector = createWarnCollector();
  const sha1ToAsset = indexAssetsBySha1(mediaMap);
  let browserTemplate;
  const raw = await withChromiumPage(
    htmlPath,
    { width: CANVAS_W, height: CANVAS_H },
    async (page) => {
      const srcToAsset = await mapImageSrcsToAssets(page, sha1ToAsset);
      await inlineSvgImages(page, srcToAsset);
      const extracted = await extractSlides(page);
      remapExtractedImageSrcs(extracted.slides, srcToAsset);
      await rewriteDomImageSrcs(page, srcToAsset);
      browserTemplate = await page.content();
      return extracted;
    },
    { readySelector: 'section', fitSelector: 'section' },
  );
