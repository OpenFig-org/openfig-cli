...
import { encodeVectorNetworkBlob } from 'openfig-core';
...
/**
 * Build a vectorNetworkBlob from SVG-style path commands.
 *
 * Delegates to `openfig-core`, which holds the encoder to Figma's verified layout
 * with a byte-identical round-trip over a 42-blob reference corpus. This package
 * previously carried its own copy; the two drifted apart once already, and the
 * duplicate is what let a wrong layout ship in one of them while the other was
 * correct.
 *
 * @param allPathCmds  one entry per path; each becomes a region
 * @param emitRegions  false for open stroked paths (addPath). A region asks Figma
 *                     to fill the bounded area, which on an open path closes it
 *                     visually as a lens between the endpoints even with no fill
 *                     paint. addSVG needs regions; addPath must not have them.
 */
function _buildVectorNetworkBlob(allPathCmds, { emitRegions = true } = {}) {
  return encodeVectorNetworkBlob(allPathCmds, { emitRegions });
}
