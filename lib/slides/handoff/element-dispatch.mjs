 ...
 */
export function toneAdjustmentsForBrightness(multiplier) {
  const [exposureCorrection, highlights, shadows] =
    brightnessToneRefinement(multiplier);
  const exposure = Math.max(
    -1,
    Math.min(1, exposureForBrightness(multiplier) + exposureCorrection),
  );
  const out = { exposure: +exposure.toFixed(4) };
  if (highlights !== 0) out.highlights = +highlights.toFixed(4);
  if (shadows !== 0) out.shadows = +shadows.toFixed(4);
  return out;
}
