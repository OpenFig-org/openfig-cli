...
async function buildOverlayPng(renderedPng, refPath) {
  ...
  for (let p = 0; p < totalPixels; p++) {
    ...
    if (Math.max(dr, dg, db) > DIFF_THRESHOLD) offCount++;
  ...
  return { pngBuf, offCount, offPct, totalPixels };
}
...
export async function buildReportRow({ slideNumber, renderedPng, refPath, score, scoreStr }) {
  ...
  return { n: slideNumber, scoreStr: resolvedScoreStr, renderUri, renderDims, refUri, refDims, overlayUri, offCount, offPct };
}
...
${rows.map(({ n, scoreStr, renderUri, renderDims, refUri, refDims, overlayUri, offCount, offPct }) => {
  ...
  const diffHtml = offCount > 0 ? `<div class="dims" style="color:#fa0">${offCount.toLocaleString()} px off (${offPct}%)</div>` : '<div class="dims" style="color:#6f6">0 px off</div>';
  return `
