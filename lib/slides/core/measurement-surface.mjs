...
async function clampWeightsToAvailableFaces(surface, host) {
  ...
  try {
    clamped = await surface.evaluate(({ realm }) => {
      ...
      const nearestAvailable = (entry, want) => {
        ...
        const candidates = [...entry.exact];
        for (const [lo, hi] of entry.ranges) candidates.push(lo, hi);
  ...
  if (clamped?.length && typeof host.log === 'function') {
    for (const { family, want, got, italic, count } of clamped) {
      if (italic) {
        host.log(
      ...
      host.log(
        `convert-html: ${family} has no weight ${want}; the ${count} element${count === 1 ? '' : 's'} ` +
        `asking for it are measured and written as ${got}. The browser fakes the missing weight, ` +
        'Figma does not — naming it would open the missing-font dialog.',
      );
    }
  }
}
