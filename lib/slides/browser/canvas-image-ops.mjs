...
async function decode(src, size) {
  ...
  try {
    ...
    // `img.decode()` has no timeout and, for some SVG sources, never settles —
    // it neither resolves nor rejects. With nothing racing it the whole
    // conversion stops dead: no error, no progress, and eventually a frozen
    // tab, which is precisely how this was found. `onerror` is raced too,
    // because a source that fails to load may never reject the decode either.
    await Promise.race([
      img.decode(),
