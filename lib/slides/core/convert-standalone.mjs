...
//
// Returns one of:
//   { forceWhite: true, raw }        `brightness(0) invert(1)` in any order —
...
//
// `css` is the original string, which a Canvas can apply verbatim through
// `ctx.filter`; `ops` is the same chain decomposed for a host that has to
// compose the maths itself. Both describe the same transform, so the two hosts
// agree without either having to parse CSS.
...
function parseImageFilter(raw) {
  ...
  const brightnessZero = ops.some((o) => o.fn === 'brightness' && o.amount === 0);
  if (ops.length === 2 && brightnessZero && invert) return { forceWhite: true, raw };
  ...
  for (const slide of manifestOut.slides) {
    for (const el of slide.elements ?? []) {
      if ((el.type === 'image' || el.type === 'svg') && el.unhandledFilter) {
        ...
        collector.warn(
          slide.index - 1,
          `${what} filter not applied: ${el.unhandledFilter} (supported: grayscale, brightness, contrast, invert, alone or chained)`,
